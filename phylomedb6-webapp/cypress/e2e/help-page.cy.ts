const HELP_PAGE_URL = "http://localhost:3050/help";
...
describe("help-page tests", () => {
    ...
    it("Add new guide", () => {
        ...
        cy.wait(3000);
        cy.visit(HELP_PAGE_URL);
        cy.get('[data-cy="add-new-guide-button"]').click();
        cy.get('[data-cy="new-title-input"]').clear().type("testestest");
        cy.get(".ProseMirror").clear().type("testytesty");
        cy.get('[data-cy="save-help-guide-button"]').click();
        cy.get('[data-cy="success-window"]').should(
            "contain",
            "Successfully created new help guide!",
        );
    });
    ...
    it("Add existing guide", () => {
        ...
        cy.get('[data-cy="error-message-window"]').should(
            "contain",
            "Guide with title: testestest, already exists in the database.",
        );
    });
    ...
    it("back to users guide button ", () => {
        ...
        cy.get('[data-cy="new-title-input"]').clear().type("testestest");
        cy.get(".ProseMirror").clear().type("testytesty");
        cy.get('[data-cy="back-to-users-guide"]').click();
        cy.url().should("contain", "http://localhost:3050/help");
    });
});
describe("edit guide button", () => {
    ...
    it("Edit title in help page", () => {
        ...
        cy.get('[data-cy="success-window"]').should(
            "contain",
            "Successfully edited help guide!",
        );
    });

    it("Edit title in help page inserting existing title", () => {
        cy.visit(`${LOGIN_URL}?login_token=${TEST_TOKEN}`);
        cy.wait(3000);
        cy.visit(HELP_PAGE_URL);
    ...
    it("Edit title in help page inserting empty title", () => {
        cy.visit(`${LOGIN_URL}?login_token=${TEST_TOKEN}`);
        cy.wait(3000);
        cy.visit(HELP_PAGE_URL);
        cy.contains("button", "TTTT")
    ...
    it("Delete guide test", () => {
        ...
        cy.visit("http://localhost:3050/help");
