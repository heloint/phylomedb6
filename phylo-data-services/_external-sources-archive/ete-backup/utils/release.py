...
a, b, c, tag, ncom, hcom = re.search(
    "(\d+)\.(\d+)\.(\d+)(-?\w+\d+)?-?(\d+)?-?(\w+)?", CURRENT_VERSION
).groups()
a, b, c = map(int, (a, b, c))
...
if not options.doconly:
    ...
    if ask('Write "%s" and commit changes?' % NEW_VERSION, ["y", "n"]) == "y":
        ...
        open("../ete3/version.py", "w").write(
            "#autogenerated during release process. Do not modify\n__version__='%s'\n"
            % (NEW_VERSION)
        )
        _ex(
            'git commit -a -m "release %s " && git tag -f %s'
    ...
    if not options.notest:
        _ex(
            'cd release/dist/ && tar xf ete3-%s.tar.gz && cd ete3-%s/ && PYHTONPATH="./" python -m "ete3.test.test_all"'
            % (NEW_VERSION, NEW_VERSION)
        )

    if ask("Upload to TEST pypi?", ["y", "n"]) == "y":
        _ex(
            "cd release/ && python setup.py sdist upload -r https://testpypi.python.org/pypi"
        )

    if ask("Upload to pypi?", ["y", "n"]) == "y":
        _ex(
            "cd release/ && python setup.py sdist upload -r https://pypi.python.org/pypi"
        )
