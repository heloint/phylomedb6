...
def print_table(
    ...
    if isinstance(fix_col_width, list):
        ...
        c2maxw = {
            i: max([safelen(str(e[i])) for e in items]) for i in range(len(items[0]))
        }
    else:
        c2maxw = {
            i: min(max_col_width, max([safelen(str(e[i])) for e in items]))
...
def ask(string, valid_values, default=-1, case_sensitive=False):
    ...
    while True:
        ...
        if case_sensitive and answer in valid_values:
            return answer
        elif not case_sensitive and answer.lower() in [v.lower() for v in valid_values]:
            return answer.lower()
