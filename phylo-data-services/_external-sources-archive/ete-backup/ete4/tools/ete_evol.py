...
DESC = (
    "Run/Load evolutionary tests, store results in a given oputput folder\n"
    "********************************************************************"
)
...
=========== ============================= ==================
    ...
    evol_args.add_argument(
        ...
        action="store_true",
        help=(
            "Prints the correspondence between PAML "
    ...
    evol_args.add_argument(
        ...
        nargs="+",
        help=(
            "directory where pre-calculated models are "
    ...
    codeml_mk.add_argument(
        ...
        action="store_true",
        help=(
            "open the input tree in GUI to allow to "
    ...
    codeml_mk.add_argument(
        ...
        action="store_true",
        help=(
            "Mark successively all the leaves of the input "
    ...
    codeml_mk.add_argument(
        ...
        action="store_true",
        help=(
            "Mark successively all the internal node of "
    ...
    codeml_ts.add_argument(
        ...
        type=str,
        help=(
            "Defines the set of tests to perform.\n"
    ...
    params = "".join(
        "[%4s] %-13s" % (PARAMS[p], p) + ("" if i % 4 else "\n")
        for i, p in enumerate(sorted(PARAMS, key=lambda x: x.lower()), 1)
    )

    codeml_gr.add_argument(
        "--codeml_config_file",
        dest="config_file",
        metavar="",
        default=None,
        help=(
            "CodeML configuration file to be used instead"
    ...
    codeml_gr.add_argument(
        ...
        default=[],
        help=(
            "extra parameter to be interpreted by CodeML "
    ...
    codeml_gr.add_argument(
        ...
        dest="super_help",
        help=(
            "show detailed description on codeml "
    ...
    img_gr.add_argument(
        ...
        action="store_true",
        help=(
            "Opens ETE interactive GUI to visualize tree and "
    ...
    img_gr.add_argument(
        "--noimg", dest="noimg", action="store_true", help=("Do not generate images.")
    )
    ...
    img_gr.add_argument(
        ...
        dest="clean_layout",
        action="store_true",
        help=("Other visualization option, with omega values " "written on branches"),
    )
    ...
    img_gr.add_argument(
        ...
        type=str,
        nargs="+",
        choices=["bar", "stick", "curve", "+-bar", "+-stick", "+-curve"],
        help=(
            "Type of histogram face to be used for site "
    ...
    exec_group.add_argument(
        "--slr_binary", dest="slr_binary", help="[%(default)s] path to Slr binary"
    )
    ...
    exec_group.add_argument(
        ...
        dest="resume",
        action="store_true",
        help=("Skip model if previous results are found in " "the output directory."),
    )
...
def marking_layout(node):
    ...
    if hasattr(node, "mark") and node.mark != "":
        ...
        label_face = TextFace(
            str(mark).center(3), fsize=12, fgcolor="white", ftype="courier"
        )
    ...
    if node.is_leaf:
        node.add_face(
            TextFace(" %s" % node.name, ftype="courier", fgcolor="#666666"),
            column=10,
            position="branch-right",
        )
    else:
        node.add_face(
            TextFace(" %s" % node.name, fsize=8, ftype="courier", fgcolor="#666666"),
            column=0,
            position="branch-top",
        )
...
def update_marks_from_args(nodes, marks, tree, args):
    ...
    if args.mark_internals:
        ...
        nodes.extend(
            [
                [n2.node_id for n2 in n.descendants()]
                for n in tree.descendants()
                if not n.is_leaf
            ]
        )
...
def interactive_mark(tree, mode="new"):
    ...
    if mode == "new":
        ts.title.add_face(
            TextFace("  Mark tree by clicking on nodes", fsize=14), column=0
        )
        ts.title.add_face(
            TextFace("      close window to start the analysis", fsize=12), column=0
        )
    elif mode == "check":
        ts.title.add_face(
            TextFace("  Check/change marks by clicking on nodes", fsize=14), column=0
        )
        ts.title.add_face(
            TextFace("      close window to start the analysis", fsize=12), column=0
        )
        ...
        ts.title.add_face(
            TextFace("  Continue marking for new analysis", fsize=14), column=0
        )
        ts.title.add_face(
            TextFace(
                "      if the tree is not marked, the analysis will start after",
                fsize=12,
            ),
            column=0,
        )
        ts.title.add_face(
            TextFace(
                "      closing window (new marking proposed otherwise).", fsize=12
            ),
            column=0,
        )
...
def remove_duplicated_marks(nodes, marks, tree):
    ...
    for bad in bads[::-1]:
        warn(
            "WARNING: removing duplicated mark %s"
            % (
                " ".join(
                    [
                        "%s%s"
                        % (
                            tree.get_descendant_by_node_id(nodes[bad][n]).write(
                                parser=9
                            ),
                            marks[bad][n],
                        )
...
def name_model(tree, base_name):
    ...
    return (
        base_name[:12]
        + "~"
        + md5((tree.get_topology_id("name") + base_name).encode("utf8")).hexdigest()
    )
    ...
    if os.path.exists(os.path.join(tree.workdir, dir_name, "out")):
        if modmodel != "SLR":
            ...
            if "Time used" in fhandler.read():
                results.append((os.path.join(tree.workdir, dir_name, "out"), modmodel))
                return True
        else:
...
def run_all_models(tree, nodes, marks, args, **kwargs):
    ...
    for model in args.models:
        binary = (
            os.path.expanduser(args.slr_binary)
            if model == "SLR"
        ...
        if AVAIL[model.split(".")[0]]["allow_mark"]:
            if not marks:
                if check_done(tree, model, results):
                    if args.resume:
                        print(
                            "Model %s (%s) already executed... SKIPPING"
            ...
            for mark, node in zip(marks, nodes):
                ...
                results.append(
                    pool.apply_async(
                        local_run_model,
                        args=(tree.copy(), modmodel, binary),
                        kwds=kwargs,
                    )
                )
        else:
...
def reformat_nw(nw_path):
    ...
    if os.path.exists(nw_path):
        ...
        file_string = sub(
            "'?(#[0-9]+)'?", "[&&NHX:mark=\\1]", file_string[beg : end + 1]
        )
        return file_string
    return nw_path
    ...
    for model in args.models:
        ...
        if AVAIL[model.split(".")[0]]["allow_mark"]:
            for mark, node in zip(marks, nodes):
                ...
                tree.mark_tree(node, marks=mark)
                modmodel = (
                    model + "." + "_".join([str(n) for n in node])
                )  # check out changes in other function
                if check_done(tree, modmodel, results):
                    continue
                else:
...
def load_model(model_name, tree, path, **kwargs):
    ...
    try:
        ...
        raise (
            Exception(
                "ERROR: model %s failed, problem with outfile:\n%s"
                % (model_obj.name, path)
            )
        )
...
def write_results(tree, args):
    ...
    for null in tree._models:
        for altn in tree._models:
            ...
            if (
                (
                    AVAIL[null.split(".")[0]]["typ"] == "site"
                    and AVAIL[altn.split(".")[0]]["typ"] == "branch"
                )
                or (
                    AVAIL[altn.split(".")[0]]["typ"] == "site"
                    and AVAIL[null.split(".")[0]]["typ"] == "branch"
                )
            ) and args.tests == "auto":
                continue
            ...
            if args.tests != "auto":
                if not any(
                    (null.split(".")[0] in test and altn.split(".")[0] in test)
                    for test in wanted
                ):
                    continue
            ...
            tests += "     %46s |%46s | %f%s\n" % (
                name_model(tree, null),
                name_model(tree, altn),
                results[(null, altn)],
                (
                    "**"
                    if results[(null, altn)] < 0.01
                    else "*" if results[(null, altn)] < 0.05 else ""
                ),
            )
...
def run(args):
    ...
    if not os.path.exists(binary):
        print("Warning: SLR binary does not exist at %s" % args.slr_binary, file=stderr)
        print(
            "         provide another route with --slr_binary, or install "
            "it by executing 'ete3 install-external-tools paml'",
            file=stderr,
        )
        if args.models and any(
            AVAIL[m.split(".")[0]]["exec"] == "Slr" for m in args.models
        ):
            return
    binary = os.path.expanduser(args.codeml_binary)
    if not os.path.exists(binary):
        print(
            "Warning: CodeML binary does not exist at %s" % args.codeml_binary,
            file=stderr,
        )
        print(
            "         provide another route with --codeml_binary, or install "
            "it by executing 'ete3 install-external-tools paml'",
            file=stderr,
        )
    ...
    if args.super_help:
        help_str = (
            "Description of CodeML parameters, see PAML manual for more "
            "information\n\n"
        )
    ...
    if args.config_file:
        if args.params:
            print(
                "WARNING: input CodeML parameters from configuration file will"
                " be overridden by the ones in the command line"
            )
        params = parse_config_file(args.config_file)
        if "seqfile" in params:
            args.alg = os.path.join(
                os.path.split(args.config_file)[0], params["seqfile"]
            )
            del params["seqfile"]
        if "outfile" in params:
            if not args.output:
                args.output = os.path.join(
                    os.path.split(args.config_file)[0], params["outfile"]
                )
            else:
                print(
                    "WARNING: input CodeML output file from configuration file"
                    " will be overridden by the one in the command line"
                )
            del params["outfile"]
        if "treefile" in params:
            if not args.src_trees:
                args.src_tree_iterator = [
                    os.path.join(os.path.split(args.config_file)[0], params["treefile"])
                ]
            else:
                args.src_tree_iterator = list(args.src_tree_iterator)
                print(
                    "WARNING: input CodeML tree file from configuration file"
                    " will be overridden by the one in the command line"
                )
            del params["treefile"]
        try:
            if len(args.models) > 1 or not args.models[0].startswith("XX."):
                raise Exception(
                    'ERROR: only 1 model name starting with "XX." '
    ...
    for nw in args.src_tree_iterator:
        ...
        if args.node_ids:
            ...
            for n in tree.descendants():
                ...
                print(
                    "   %-4s : %s"
                    % (n.node_id, ", ".join([l.name for l in n.leaves()]))
                )
        ...
        if args.prev_models:
            models = {
                m.split(",")[1]: m.split(",")[0] + "/out" for m in args.prev_models
            }
        # run models
        if args.models:
        ...
        try:
            best = max(
                [m for m in bests if AVAIL[m.split(".")[0]]["typ"] == "branch"],
        ...
        site_models = [
            m
            for m in tree._models
            if (
                "site" in AVAIL[m.split(".")[0]]["typ"]
        ...
        for model in tree._models.values():
            ...
            if any(model.branches[b]["mark"] for b in model.branches):
                ...
                omega_mark = [
                    (model.branches[b]["w"], model.branches[b]["mark"].strip())
                    for b in model.branches
                ...
                if omega_mark:
                    ...
                    for omega, mark in set(omega_mark):
                        print(
                            "      %10s  => %7.3f"
                            % (mark.replace("#0", "background"), omega)
                        )
                clean_tree(tree)
        ...
        if args.histface:
            if len(args.histface) != len(site_models):
                if len(args.histface) == 1:
                    ...
                    args.histface.extend(
                        [args.histface[-1]] * (len(site_models) - len(args.histface))
                    )
                else:
            ...
            for num, (hist, model) in enumerate(zip(args.histface, site_models)):
                ...
                model.set_histface(
                    up=not bool(num), kind=hist.replace("+-", ""), errors="+-" in hist
                )
