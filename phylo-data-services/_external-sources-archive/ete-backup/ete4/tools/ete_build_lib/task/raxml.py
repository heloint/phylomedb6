...
from ..utils import basename, Tree, OrderedDict, GLOBALS, pjoin, DATATYPES, md5
from .. import db
...
class Raxml(TreeTask):
    def __init__(
        ...
        if model and model.startswith("pmodeltest-"):
            ...
            if seqtype == "nt" and basemodel != "GTR":
                log.warning(
                    "Raxml supports only the GTR model, but model selection returned %s. Consider using Phyml if this is important."
                    % basemodel
                )
        ...
        TreeTask.__init__(self, nodeid, "tree", "RaxML", base_args, conf[confname])
    ...
    def load_jobs(self):
        ...
        if self.bootstrap == "alrt":
            ...
            alrt_job = Job(
                self.conf["app"]["phyml"], alrt_args, parent_ids=[tree_job.jobid]
            )
            ...
            boot_args["-b"] = 31416
            boot_job = Job(self.raxml_bin, boot_args, parent_ids=[tree_job.jobid])
            ...
            bootd_args["-z"] = pjoin(
                boot_job.jobdir, "RAxML_bootstrap." + boot_job.args["-n"]
            )

            bootd_job = Job(self.raxml_bin, bootd_args, parent_ids=[tree_job.jobid])
    ...
    def finish(self):
        ...
        if self.bootstrap == "alrt":
            alrt_tree_file = os.path.join(
                self.alrt_job.jobdir,
                "RAxML_fastTreeSH_Support." + self.alrt_job.args["-n"],
            )
            raw_nw = open(alrt_tree_file).read()
            try:
                nw, nsubs = re.subn(
                    ":(\d+\.\d+)\[(\d+)\]", parse_alrt, raw_nw, flags=re.MULTILINE
                )
            except TypeError:
            ...
            alrt_tree_file = os.path.join(
                self.alrt_job.jobdir, self.alg_phylip_file + "_phyml_tree.txt"
            )
            tree = Tree(alrt_tree_file)

        else:
            alrt_tree_file = os.path.join(
                self.bootd_job.jobdir, "RAxML_bipartitions." + self.bootd_job.args["-n"]
            )
