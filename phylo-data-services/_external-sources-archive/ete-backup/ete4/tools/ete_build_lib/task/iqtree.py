...
class IQTree(TreeTask):
    def __init__(
        ...
        TreeTask.__init__(self, nodeid, "tree", "IQTree", base_args, conf[confname])
    ...
    def finish(self):
        j = self.jobs[0]
        tree_file = os.path.join(j.jobdir, self.alg_phylip_file + ".treefile")
        stats_file = os.path.join(j.jobdir, self.alg_phylip_file + ".iqtree")

        m = re.search(
            "Log-likelihood of the tree:\s+(-?\d+\.\d+)", open(stats_file).read()
        )
        lk = float(m.groups()[0])
        stats = {"lk": lk}
