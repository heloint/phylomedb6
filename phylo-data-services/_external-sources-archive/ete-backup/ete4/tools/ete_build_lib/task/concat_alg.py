    ...
    def finish(self):
        ...
        for job in self.jobs:
            ...
            if job.ttype == "alg" and job.nodeid not in self.job2alg:
                ...
                clean_model = (
                    job.best_model.replace("pmodeltest-", "")
                    .split("+")[0]
                    .split("!")[0]
                )
        ...
        if self.cog_ids - set(self.job2alg):
            log.error("Missing %s algs", len(self.cog_ids - set(self.job2alg)))
            missing = self.cog_ids - set(self.job2alg)
            raise TaskError(
                self, "Missing algs (%d): i.e. %s" % (len(missing), missing[:10])
            )

        alg_data = [
            (self.job2alg[nid], self.job2model.get(nid, self.default_model))
        ...
        mainalg, partitions, sp2alg, species, alg_lenghts = get_concatenated_alg(
            filenames, models, sp_field=0, sp_delimiter=GLOBALS["spname_delimiter"]
        )
...
def get_concatenated_alg(
    ...
    valid_species = [
        sp
        for sp in sp2alg.keys()
        if sp in keep_species or len(sp2alg[sp]) / float(len(alg_objects)) > kill_thr
    ]

    log.info(
        "%d out of %d will be kept (missing factor threshold=%g, %d species forced to kept)"
