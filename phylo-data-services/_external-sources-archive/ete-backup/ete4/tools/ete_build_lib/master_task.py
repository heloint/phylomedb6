...
def genetree_class_repr(cls, cls_name):
    ...
    return "%s (%s %s seqs, %s, %s/%s)" % (
        cls_name,
        getattr(cls, "size", None) or "",
        getattr(cls, "seqtype", None) or 0,
        cls.tname,
...
class Task:
    ...
    def __init__(self, nodeid, task_type, task_name, base_args=None, extra_args=None):
    ...
    def load_task_info(self):
        ...
        if not self.taskid:
            ...
            unique_id = md5(
                ",".join(
                    [self.nodeid, self._config_id, args_id]
                    + sorted([getattr(j, "jobid", "taskid") for j in self.jobs])
                )
            )
    ...
    def iter_waiting_jobs(self):
        for j in self.jobs:
            ...
            if st == "W" and not (j.dependencies - self._donejobs):
                ...
                if isjob(j):
                    j.dump_script()
                    cmd = "sh %s >%s 2>%s" % (j.cmd_file, j.stdout_file, j.stderr_file)
                    yield j, cmd
                elif istask(j):
...
class AlgCleanerTask(Task):
    ...
    def check(self):
        if self.clean_alg_fasta_file and self.clean_alg_phylip_file:
            return True
        return False

    def init_output_info(self):
        self.clean_alg_fasta_file = "%s.%s" % (self.taskid, DATATYPES.clean_alg_fasta)
        self.clean_alg_phylip_file = "%s.%s" % (self.taskid, DATATYPES.clean_alg_phylip)
        self.kept_columns = []
...
class TreeTask(Task):
    def __init__(self, nodeid, task_type, task_name, base_args=None, extra_args=None):
        ...
        Task.__init__(self, nodeid, task_type, task_name, base_args, extra_args)
...
def update_task_states_recursively(task):
    ...
    db.update_task(
        task.taskid, status=task.status, tm_start=task_start, tm_end=task_end
    )
