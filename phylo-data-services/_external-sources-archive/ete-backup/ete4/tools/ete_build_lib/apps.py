...
APPTYPES = {
    "aligners": set(
        ["muscle", "mafft", "clustalo", "metaligner", "dialingtx", "tcoffee"]
    ),
...
def test_apps(apps):
    ...
    for name, cmd in sorted(apps.items()):
        if name == "dialigntx" and sys.platform == "darwin":
            print(
                colorify("Dialign-tx not supported in OS X", "orange"), file=sys.stderr
            )
            continue
    ...
    if errors:
        print(
            colorify(
                "\nWARNING: %d external tools seem to be missing or unfunctional"
                % errors,
                "yellow",
            ),
            file=sys.stderr,
        ...
        print(
            colorify(" conda install -c etetoolkit ete3_external_apps", "white"),
            file=sys.stderr,
        )
