    ...
    for c1, (raw_seqname, seq) in enumerate(iter_fasta_seqs(seqfile)):
        if c1 % 10000 == 0 and c1:
            if loaded_seqs and target_seqs:  # only works when workflow is supermatrix
                estimated_time = (
                    (len(target_seqs) - len(loaded_seqs)) * (time.time() - start_time)
                ) / float(c1)
                ...
                estimated_time = -1
            print(
                "loaded:%07d skipped:%07d scanned:%07d %0.1f%%"
                % (len(loaded_seqs), skipped_seqs, c1, percent),
                end="\n",
                file=sys.stderr,
            )
        ...
        if target_seqs and loaded_seqs == len(target_seqs):
            ...
            target_species
            and seqname.split(args.spname_delimiter, 1)[0] not in target_species
        ):
            skipped_seqs += 1
            continue
        ...
        if cached_seqs:
            try:
                ...
                raise DataError(
                    "%s sequence not found in %s sequence file" % (seqname, seqtype)
                )
        else:
        ...
        if seqname in loaded_seqs:
            if fix_dups:
                ...
                raise DataError(
                    "Duplicated sequence name [%s] found. Fix manually or use --rename-dup-seqnames to continue"
                    % (seqname)
                )
...
def check_seq_integrity(
    args, target_seqs, visited_seqs, seq2length, seq2unknown, seq2seq
):
    ...
    for seqtype in GLOBALS["seqtypes"]:
        if seq2unknown[seqtype]:
            ...
            error += "\n".join(
                [
                    "%s\tcontains:\t%s" % (k, " ".join(v))
                    for k, v in seq2unknown[seqtype].items()
                ]
            )
    ...
    if GLOBALS["seqtypes"] == set(["aa", "nt"]):
        ...
        for seqname, ntlen in seq2length["nt"].items():
            if seqname in seq2length["aa"]:
                ...
                if ntlen / 3.0 != aa_len:
                    inconsistent_cds.add(
                        "%s\tExpected:%d\tFound:%d" % (seqname, aa_len * 3, ntlen)
                    )
                else:
                    if not args.no_seq_checks:
                        for i, aa in enumerate(seq2seq["aa"][seqname]):
                            ...
                            if not (set(codon) - REAL_NT):
                                if GENCODE[codon] != aa:
                                    log.warning(
                                        "@@2:Unmatching codon in seq:%s, aa pos:%s (%s != %s)@@1: Use --no-seq-checks to skip"
                                        % (seqname, i, codon, aa)
                                    )
                                    inconsistent_cds.add(
                                        "Unmatching codon in seq:%s, aa pos:%s (%s != %s)"
                                        % (seqname, i, codon, aa)
                                    )
...
def hash_names(target_names):
    ...
    if collisions:
        ...
        for old_hash, coliding_names in collisions:
            ...
            log.log(
                20,
                "Collision found when hash-encoding the following gene names: %s",
                coliding_names,
            )
            niter = 1
            valid = False
            ...
            log.log(
                20,
                "Fixed with %d concatenations! %s",
                niter,
                ", ".join(["%s=%s" % (e[1][0], e[0]) for e in new_hashes.items()]),
            )
