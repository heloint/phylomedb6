...
def zencode(x, data_id):
    ...
    if sys.getsizeof(pdata) > MAX_SQLITE_SIZE:
        ...
        pickle.dump(
            x, open(pjoin(GLOBALS["db_dir"], data_id + ".pkl"), "wb"), protocol=1
        )
...
def parse_job_list(jobs):
    if isjob(jobs) or istask(jobs):
        jobs = [jobs]
    ids = ",".join(
        ["'%s'" % j.jobid for j in jobs if isjob(j)]
...
def add_task(tid, nid, parent=None, status=None, type=None, subtype=None, name=None):
    values = ",".join(
        ['"%s"' % (v or "") for v in [tid, nid, parent, status, type, subtype, name]]
    )
    cmd = (
        "INSERT OR REPLACE INTO task (taskid, nodeid, parentid, status,"
...
def add_runid2task(runid, tid):
    cmd = (
        "INSERT OR REPLACE INTO runid2task (runid, taskid)"
...
def get_runid_tasks(runid):
    ...
    cmd = "SELECT taskid FROM runid2task" ' WHERE runid = "%s";' % (runid)
    execute(cmd)
...
def update_task(tid, **kargs):
    if kargs:
        values = ", ".join(['%s="%s"' % (k, v) for k, v in kargs.items()])
...
def get_sge_tasks():
    cmd = (
        'SELECT taskid, pid FROM task WHERE host="@sge" '
        ' AND status IN ("Q", "R", "L");'
    )
    execute(cmd)
...
def add_node(runid, nodeid, cladeid, targets, outgroups):
    ...
    cmd = (
        "INSERT OR REPLACE INTO node (nodeid, cladeid, target_seqs, out_seqs,"
...
def get_node_info(threadid, nodeid):
    cmd = (
        "SELECT cladeid, target_seqs, out_seqs FROM"
        ' node WHERE runid="%s" AND nodeid="%s"' % (threadid, nodeid)
    )
...
def print_node_by_clade(threadid, cladeid):
    cmd = (
        "SELECT nodeid, target_seqs, out_seqs, newick FROM"
        ' node WHERE runid="%s" AND cladeid="%s"' % (threadid, cladeid)
    )
...
def get_runid_nodes(runid):
    cmd = (
        "SELECT cladeid, newick, target_size FROM node"
...
def report(runid, filter_rules=None):
    ...
    cmd = (
        "SELECT task.taskid, task.nodeid, task.parentid, node.cladeid, task.status, type, subtype, name,"
...
def add_seq_name(seqid, name):
    cmd = "INSERT OR REPLACE INTO seqid2name (seqid, name)" ' VALUES ("%s", "%s");' % (
        seqid,
        name,
    )
...
def add_seq(seqid, seq, seqtype):
    cmd = 'INSERT OR REPLACE INTO %s_seq (seqid, seq) VALUES ("%s", "%s")' % (
        seqtype,
        seqid,
        seq,
    )
...
def add_seq_species(species):
    ...
    seqcursor.executemany(cmd, [[sp, counter] for sp, counter in species.items()])
    autocommit()
