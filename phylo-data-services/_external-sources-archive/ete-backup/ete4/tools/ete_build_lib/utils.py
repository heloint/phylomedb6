...
random_string = lambda N: "".join(
    random.choice(string.ascii_uppercase + string.digits) for x in range(N)
)
...
def print_as_table(
    rows, header=None, fields=None, print_header=True, stdout=sys.stdout, wide=True
):
    ...
    if vtype == list or vtype == tuple:
        ...
        for r in rows:
            for i, iv in enumerate(fields):
                ...
                print(
                    _safe_rjust(_str(r[iv]), lengths[i]) + " | ", end=" ", file=stdout
                )
            print("", file=stdout)
        ...
        for ppt in header:
            lengths[ppt] = max(
                [_safe_len(_str(ppt))] + [_safe_len(_str(p.get(ppt, ""))) for p in rows]
            )
        if header:
            for ppt in header:
                print(
                    _safe_rjust(_str(ppt), lengths[ppt]) + " | ", end=" ", file=stdout
                )
            print("", file=stdout)
            for ppt in header:
        ...
        for p in rows:
            for ppt in header:
                print(
                    _safe_rjust(_str(p.get(ppt, ""), lengths[ppt])) + " | ",
                    end=" ",
                    file=stdout,
                )
            print("", file=stdout)
...
def npr_layout(node):
    ...
    if "alg_type" in node.features:
        faces.add_face_to_node(
            faces.AttrFace("alg_type", fsize=8), node, 0, position="branch-top"
        )

    if "treemerger_rf" in node.features:
        faces.add_face_to_node(
            faces.AttrFace("treemerger_rf", fsize=8), node, 0, position="branch-bottom"
        )
    ...
    if support_radius > 1:
        ...
        faces.add_face_to_node(
            faces.AttrFace("support", fsize=8), node, 0, position="branch-bottom"
        )
...
def iter_cog_seqs(cogs_file, spname_delimiter):
    ...
    for line in open(cogs_file):
        ...
        for seqname in [_ln.strip() for _ln in line.split("\t")]:
            try:
                spcode, seqcode = [
                    _f.strip() for _f in seqname.split(spname_delimiter, 1)
                ]
            except ValueError:
