...
def annotate_node(t, final_task):
    ...
    for task in alltasks:
        ...
        if task.ttype == "msf":
            n.add_properties(msf_outseqs=task.out_seqs, msf_file=task.multiseq_file)

        elif task.ttype == "acleaner":
            n.add_properties(
                clean_alg_mean_ident=task.mean_ident,
            ...
            n.add_properties(
                alg_mean_ident=task.mean_ident,
...
def get_trimal_conservation(alg_file, trimal_bin):
    output = bytes.decode(
        check_output("%s -ssc -in %s" % (trimal_bin, alg_file), shell=True)
    )
    conservation = []
...
def get_statal_identity(alg_file, statal_bin):
    output = bytes.decode(
        check_output("%s -scolidentt -in %s" % (statal_bin, alg_file), shell=True)
    )
...
def get_trimal_identity(alg_file, trimal_bin):
    ...
    #    (trimal_bin, alg_file)
    output = byets.decode(
        check_output("%s -sident -in %s" % (trimal_bin, alg_file), shell=True)
    )
    # print output
    conservation = []
    for line in output.split("\n"):
...
def process_task(task, wkname, npr_conf, nodeid2info):
    ...
    if ttype == "msf":
        ...
        # Register node
        db.add_node(
            task.threadid, task.nodeid, task.cladeid, task.target_seqs, task.out_seqs
        )
        ...
        if alignerclass:
            alg_task = alignerclass(
                nodeid, task.multiseq_file, seqtype, conf, alignerconf
            )
        else:
            log.warning("Skipping alignment phase, using original sequences")
            alg_task = ManualAlg(nodeid, task.multiseq_file, seqtype, conf, alignerconf)
        ...
        if seqtype == "aa" and npr_conf.switch_aa_similarity < 1:
            ...
            if "i_mean" not in alg_stats:
                ...
                mx, mn, mean, std = get_statal_identity(algfile, conf["app"]["statal"])
        ...
        if ttype == "alg" and cleanerclass:
            next_task = cleanerclass(
                nodeid, seqtype, alg_fasta_file, alg_phylip_file, conf, cleanerconf
            )
        else:
            # Converts aa alignment into nt if necessary
            if (
                seqtype == "aa"
                and "nt" in GLOBALS["seqtypes"]
                and task.mean_ident >= npr_conf.switch_aa_similarity
            ):
                log.log(
                    28,
                    "@@2:Switching to codon alignment!@@1: amino-acid sequence similarity: %0.2f >= %0.2f"
        ...
        treemerge_task = splitterclass(
            nodeid, seqtype, task.tree_file, conf, splitterconf
        )
        ...
        if not isinstance(treebuilderclass, DummyTree) and npr_conf.max_iters > 1:
            ...
            if npr_conf.max_iters and current_iter >= npr_conf.max_iters:
                ...
                for node, seqs, outs, wkname in get_next_npr_node(
                    threadid, ttree, task.out_seqs, mtree, alg_path, npr_conf
                ):
                    log.log(
                        24,
                        "Registering new node: %s seqs, %s outgroups",
                        len(seqs),
                        len(outs),
                    )
