...
class IterConfig(dict):
    ...
    def __getitem__(self, v):
        ...
        if v in set(["tree_builder", "aligner", "model_tester", "alg_cleaner"]):
...
def split_tree(
    ...
    # same tree
    tasktree_content = set([leaf.name for leaf in n2content[task_tree_node]]) | set(
        task_outgroups
    )
    while trees_to_browse:
                ...
                if not optimized:
                    ...
                    if ancestor in avail_nodes:
                        ...
                        if not strict_monophyly or lin2sp[lin] == ancestor_content:
                            ...
                            log.log(
                                26,
                                "Discarding not monophyletic level @@11:%s@@1:" % lin,
                            )
                    else:
        ...
        for node in master_node.iter_leaves(is_leaf_fn=processable_node):
            if opt_levels:
                log.log(
                    28,
                    "Trying to optimizing custom tree level: @@11:%s@@1:"
                    % _TARGET_NODES[node],
                )
            ...
            log.log(
                28,
                "Found possible target node of size %s branch support %f"
...
def get_next_npr_node(
    threadid, ttree, task_outgroups, mtree, alg_path, npr_conf, target_cladeids=None
):
    current_iter = get_iternumber(threadid)
    ...
    for node, seqs, outs, wkname in split_tree(
        ttree, task_outgroups, mtree, alg_path, npr_conf, threadid, target_cladeids
    ):
        if npr_conf.max_iters and current_iter < npr_conf.max_iters:
            log.log(
                28,
...
def select_closest_outgroup(target, n2content, splitterconf):
    ...
    max_outgroup_size = max(
        int(float(splitterconf["_max_outgroup_size"]) * len(n2content[target])), 1
    )
    ...
    n2targetdist = distance_matrix_new(
        target, leaf_only=False, topological=out_topodist
    )
...
def select_outgroups(target, n2content, splitterconf):
    ...
    score = lambda _n: (
        ...
        1
        - (
            abs(optimal_out_size - len(n2content[_n]))
            / float(max(optimal_out_size, len(n2content[_n])))
        ),  # outgroup size
    ...
    valid_nodes = [
        ...
        if not n2content[n] & n2content[target] and n.support >= out_min_support
    ]
    if not valid_nodes:
        raise TaskError(
            None,
            "Could not find a suitable outgroup (min_support=%s)" % out_min_support,
        )
