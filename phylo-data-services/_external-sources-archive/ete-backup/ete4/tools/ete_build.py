...
from .ete_build_lib.utils import (
    SeqGroup,
    generate_runid,
    AA,
    NT,
...
from .ete_build_lib.configcheck import (
    is_file,
    is_dir,
    check_config,
...
def main(args):
    ...
    if pexist(base_dir):
        if hascontent(local_conf_file):
            if not args.clearall and not args.resume:
                raise ConfigError(
                    "Output directory seems to contain"
    ...
    def parse_workflows(names, target_wtype, parse_filters=False):
        ...
        for wkname in names:
                    ...
                    for f in fields[:-1]:
                        if f.startswith("size-range:"):  # size filter
                            ...
                            try:
                                ...
                                raise ConfigError(
                                    "size filter should consist of two integer numbers (i.e. 50-100). Found [%s] instead"
                                    % f
                                )
                            ...
                            try:
                                ...
                                raise ConfigError(
                                    "sequence similarity filter should consist of two float numbers between 0 and 1 (i.e. 0-0.95). Found [%s] instead"
                                    % f
                                )
            ...
            if target_wtype == "genetree" and wkname in base_config.get(
                "genetree_meta_workflow", {}
            ):
                temp_workflows = [
                    x.lstrip("@") for x in base_config["genetree_meta_workflow"][wkname]
                ]
            elif target_wtype == "supermatrix" and wkname in base_config.get(
                "supermatrix_meta_workflow", {}
            ):
                temp_workflows = [
                    x.lstrip("@")
                    for x in base_config["supermatrix_meta_workflow"][wkname]
                ]
            else:
            ...
            for _w in temp_workflows:
                ...
                if _w not in base_config:
                    ...
                    raise ConfigError(
                        "[%s] workflow or meta-workflow name is not found in the config file."
                        % _w
                    )
                ...
                if wtype != target_wtype:
                    raise ConfigError(
                        "[%s] is not a valid %s workflow" % (wkname, target_wtype)
                    )
            ...
            if parse_filters:
                if len(temp_workflows) == 1:
                    ...
                    raise ConfigError(
                        "Meta-workflows with multiple threads are not allowed as recursive workflows [%s]"
                        % wkname
                    )
            else:
    ...
    if supermatrix_workflows and len(genetree_workflows) > 1:
        raise ConfigError(
            "A single genetree workflow must be specified when used in combination with super-matrix workflows."
        )
    ...
    if args.npr_workflows is not None:
        use_npr = True
        npr_workflows = parse_workflows(
            args.npr_workflows, WORKFLOW_TYPE, parse_filters=True
        )
    ...
    tasks_dir = (
        os.path.realpath(args.tasks_dir) if args.tasks_dir else pjoin(base_dir, "tasks")
    )
    ...
    if args.clearall:
        ...
        if args.softclear:
            ...
            shutil.rmtree(GLOBALS["tmp"]) if pexist(GLOBALS["tmp"]) else None
            (
                shutil.rmtree(GLOBALS["input_dir"])
                if pexist(GLOBALS["input_dir"])
                else None
            )
            (
                os.remove(GLOBALS["datadb_file"])
                if pexist(GLOBALS["datadb_file"])
                else None
            )
    ...
    if not args.clearall and base_dir != GLOBALS["output_dir"]:
        log.log(
            24, "Copying previous output files to scratch directory: %s..." % base_dir
        )
        try:
        ...
        try:
            shutil.copytree(
                pjoin(GLOBALS["output_dir"], "tasks/"), pjoin(base_dir, "tasks/")
            )
        except IOError as e:
            try:
                shutil.copy(
                    pjoin(GLOBALS["output_dir"], "etebuild_data.tar.gz"), base_dir
                )
            except IOError as e:
                pass
    ...
    if pexist(os.path.join(base_dir, "etebuild_data.tar.gz")):
        ...
        cmd = (
            "cd %s && gunzip -f etebuild_data.tar.gz && tar -xf etebuild_data.tar && rm etebuild_data.tar"
            % base_dir
        )
        os.system(cmd)
    ...
    if WORKFLOW_TYPE == "supermatrix" and not args.cogs_file:
        raise ConfigError(
            "Species tree workflow requires a list of COGS"
    ...
    if WORKFLOW_TYPE == "supermatrix":
        ...
        target_seqs = set()
        for cog_number, seq_cogs in iter_cog_seqs(
            args.cogs_file, args.spname_delimiter
        ):
        ...
        if target_species is not None:
            if target_species - observed_species:
                raise DataError(
                    "The following target_species could not be found in COGs file: %s"
                    % (",".join(target_species - observed_species))
                )
        else:
            target_species = observed_species
        log.warning(
            "COG file restriction: %d sequences from %s species "
            % (len(target_seqs), len(target_species))
        )
    else:
        target_seqs = None
    ...
    if not pexist(GLOBALS["seqdb_file"]):
        ...
        seqname2seqid = None
        if args.aa_seed_file:
            seqname2seqid = seqio.load_sequences(
                args, "aa", target_seqs, target_species, seqname2seqid
            )
            if not target_seqs:
        ...
        if args.nt_seed_file:
            seqname2seqid = seqio.load_sequences(
                args, "nt", target_seqs, target_species, seqname2seqid
            )
        # Integrity checks?
        pass
        ...
        if target_seqs is None:
            ...
            if target_seqs - set(seqname2seqid.keys()):
                raise DataError(
                    "The following sequence names in COGs file"
    ...
    if ERROR:
        ...
        raise DataError(
            "Errors were found while loading data. Please"
    ...
    if WORKFLOW_TYPE == "supermatrix" and args.lineages_file and TARGET_CLADES:
        ...
        avail_levels = [
            (lin, len(lin2sp[lin]))
            for lin in all_sorted_levels
            if len(lin2sp[lin]) >= 2
        ]
        log.log(
            26,
            "Available levels for NPR optimization:\n%s",
    ...
    # if no lineages file, raise an error
        ...
        raise ConfigError(
            "The use of target_levels requires a species lineage file provided through the --lineages option"
        )
    ...
    for wkname, config in run2config.items():
        ...
        open(os.path.join(config["_outpath"], "runid"), "a").write(
            "\t".join([thread_id, GLOBALS["nprdb_file"] + "\n"])
        )
        # Write command line info
        cmd_info = "\t".join(
            [start_time, thread_id, str(args.monitor), GLOBALS["cmdline"]]
        )
        open(pjoin(config["_outpath"], "command_lines"), "a").write(cmd_info + "\n")

    thread_errors = schedule(
        pipeline, pending_tasks, args.schedule_time, execution, args.debug, args.noimg
    )
    db.close()
    ...
    if not thread_errors:
        ...
        if args.compress:
            ...
            cmd = (
                "cd %s && tar --remove-files -cf etebuild_data.tar tasks/ && gzip -f etebuild_data.tar; if [ -e etebuild.log ]; then gzip -f etebuild.log; fi;"
                % GLOBALS["basedir"]
            )
            os.system(cmd)
...
def _main(arguments, builtin_apps_path=None):
    ...
    if len(arguments) == 1:
        if not pexist(APPSPATH):
            print(
                colorify("\nWARNING: external applications not found", "yellow"),
                file=sys.stderr,
            )
            print(
                colorify("Install using conda (recomended):", "lgreen"), file=sys.stderr
            )
            print(
                colorify(" conda install -c etetoolkit ete_toolchain", "white"),
                file=sys.stderr,
            )
            print(colorify("or manually compile from:", "lgreen"), file=sys.stderr)
            print(
                colorify(" https://github.com/etetoolkit/ete_toolchain", "white"),
                file=sys.stderr,
            )
            print()
    ...
    if len(arguments) > 1:
        ...
        if arguments[1] == "check":
            if not pexist(APPSPATH):
                ...
                print(
                    colorify("Install using conda (recomended):", "lgreen"),
                    file=sys.stderr,
                )
                print(
                    colorify(" conda install -c etetoolkit ete_toolchain", "white"),
            ...
            if arguments[1] == "wl":
                print(
                    colorify(
                        "WARNING: 'wl' is obsolete and will be removed in the future, use 'workflows' instead",
                        "orange",
                    ),
                    file=sys.stderr,
                )
    ...
    parser = argparse.ArgumentParser(
        description=__DESCRIPTION__ + __EXAMPLES__,
    ...
    input_group.add_argument(
        "[check | workflows | apps | show | dump | validate]",
        nargs="?",
        help=(
            "Utility commands:\n"
    ...
    input_group.add_argument(
        ...
        dest="base_config",
        type=is_file,
        default=BASEPATH + "/ete_build.cfg",
    ...
    input_group.add_argument(
        "--seq-name-parser",
        dest="seq_name_parser",
        type=str,
        help=(
            "A Perl regular expression containing a matching group, which is"
    ...
    dup_names_group.add_argument(
        "--ignore-dup-seqnames",
        dest="ignore_dup_seqnames",
        action="store_true",
        help=(
            "If duplicated sequence names exist in the input"
    ...
    input_group.add_argument(
        "--seqdb", dest="seqdb", type=str, help="Uses a custom sequence database file"
    )
    ...
    npr_group.add_argument(
        "--first-split-outgroup",
        dest="first_split",
        type=str,
        default="midpoint",
        help=(
            "EXPERIMENTAL:When used, it overrides first_split option"
    ...
    output_group.add_argument(
        "--compress",
        action="store_true",
        help="Compress all intermediate files when" " a workflow is finished.",
    )
    ...
    exec_type_group.add_argument(
        "--noexec",
        dest="no_execute",
        action="store_true",
        help=(
            "Prevents launching any external application."
    ...
    ui_group.add_argument(
        ...
        type=int,
        choices=[0, 1, 2, 3, 4],
        help="Verbosity level: 0=very quiet, 4=very " " verbose.",
    )
    ...
    if not pexist(APPSPATH):
        print(
            colorify(
                "\nWARNING: external applications directory are not found at %s"
                % APPSPATH,
                "yellow",
            ),
            file=sys.stderr,
        ...
        print(
            colorify(
                'Use "ete build install_tools" to install or upgrade tools', "orange"
            ),
            file=sys.stderr,
        )
    ...
    if not args.noimg:
        try:
            ...
            print(
                "X11 DISPLAY = %s"
                % colorify(os.environ.get("DISPLAY", "not detected!"), "yellow")
            )
