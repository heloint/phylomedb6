...
def EUCL_DIST_B(a, b, support, attr1, attr2):
    ...
    dist_a = sum(
        [
            descendant.dist
            for descendant in a[0].leaves()
            if descendant.props[attr1] in (a[1] - b[1])
        ]
    ) / len([i for i in a[0].leaves()])
    dist_b = sum(
        [
            descendant.dist
            for descendant in b[0].leaves()
            if descendant.props[attr2] in (b[1] - a[1])
        ]
    ) / len([i for i in b[0].leaves()])

    return (
        1
        - ((float(len(a[1] & b[1])) / max(len(a[1]), len(b[1]))) + abs(dist_a - dist_b))
        / 2
    )
...
def EUCL_DIST_B_ALL(a, b, support, attr1, attr2):
    ...
    dist_a = sum([descendant.dist for descendant in a[0].leaves()]) / len(
        [i for i in a[0].leaves()]
    )
    dist_b = sum([descendant.dist for descendant in b[0].leaves()]) / len(
        [i for i in b[0].leaves()]
    )
...
def EUCL_DIST_B_FULL(a, b, support, attr1, attr2):
    ...
    dist_a = sum(
        [
            descendant[1]
            for descendant in _get_leaves_paths(a[0], attr1, support)
            if descendant[0] in (a[1] - b[1])
        ]
    ) / len([i for i in a[0].leaves()])
    dist_b = sum(
        [
            descendant[1]
            for descendant in _get_leaves_paths(b[0], attr2, support)
            if descendant[0] in (b[1] - a[1])
        ]
    ) / len([i for i in b[0].leaves()])
...
def load_matrix(file, separator):
    ...
    with open(file, "r") as f:
        headers = (
            f.readline().rstrip().split(separator)[1:]
        )  # exclude empty space at the begining
...
def dict2tree(treedict, jobs=1, parallel=None):
    ...
    if parallel == "sync":
        ...
        matrix = [
            [
                pool.apply(
                    np.corrcoef, args=(treedict["dict"][col1], treedict["dict"][col2])
                )
                for col2 in treedict["headers"]
            ]
            for col1 in treedict["headers"]
        ]
        pool.close()
    ...
    if parallel == "async":
        ...
        matrix = [
            [
                (np.corrcoef(treedict["dict"][col1], treedict["dict"][col2]))[0][1]
                for col2 in treedict["headers"]
            ]
            for col1 in treedict["headers"]
        ]
...
def pearson_corr(rdict, tdict):
    ...
    rdict["dict"] = {
        header: [rdict["dict"][header][element] for element in rfilter]
        for header in rdict["headers"]
    }

    tdict["dict"] = {
        header: [tdict["dict"][header][element] for element in tfilter]
        for header in tdict["headers"]
    }
    ...
    for a in rdict["headers"]:
        for b in tdict["headers"]:
            pearson[a][b] = pearson[b][a] = (
                1 - np.corrcoef(rdict["dict"][a], tdict["dict"][b])[0][1]
            )
...
def be_distance(t1, t2, support, attr1, attr2):
    ...
    def _get_distances(leaf_distances1, leaf_distances2):
        ...
        return abs(
            sum([leaf[1] for leaf in unique_leaves1])
            - sum([leaf[1] for leaf in unique_leaves2])
        )

    return _get_distances(
        _get_leaves_paths(t1, attr1, support), _get_leaves_paths(t2, attr2, support)
    )
...
def cc_distance(t1, t2, support, attr1, attr2):
    ...
    def cophenetic_compared_matrix(t_source, t_compare, attr1, attr2, support):
        ...
        for i in set(x.props[attr2] for x in t_compare.leaves()) - set(
            x.props[attr1] for x in t_source.leaves()
        ):
            paths[i] = set()
        ...
        allleaves = sorted(
            leaf_distances.keys()
        )  # the leaves in order that we will return
...
def treediff(
    ...
    * target node, as a tree object
    ...
    if parallel == "sync":
        ...
        gen = [
            [
                pool.apply(dist_fn, args=((n1, x), (n2, y), support, prop1, prop2))
                for n2, y in parts2
            ]
            for n1, x in parts1
        ]
        ...
        gen = [
            [
                pool.apply_async(
                    dist_fn, args=((n1, x), (n2, y), support, prop1, prop2)
                )
                for n2, y in parts2
            ]
            for n1, x in parts1
        ...
        gen = (
            (dist_fn((n1, x), (n2, y), support, prop1, prop2) for n2, y in parts2)
            for n1, x in parts1
        )
    ...
    if reduce_matrix:
        ...
        new_matrix = np.empty(
            [len(rows_to_include), len(cols_to_include)], dtype=np.float32
        )
        ...
        log.info(
            "Distance matrix reduced from %dx%d to %dx%d"
    ...
    # Show only best match
        ...
        for r in range(len(matrix)):
            ...
            if np.percentile(matrix, 5) >= matrix[r][c]:
                ...
                dist, side1, side2, diff, n1, n2 = (
                    matrix[r][c],
                    [l.name for l in parts1[r][0].leaves()],
                    [l.name for l in parts2[r][0].leaves()],
                    parts1[r][1].symmetric_difference(parts2[c][1]),
                    parts1[r][0],
                    parts2[c][0],
                )
    ...
    if extended:
        return "\n".join(
            [
                ...
                "%0.6f\t%0.6f\t%10d\t%d\t%d"
                % (total_dist, total_bdist, len(difftable), rf, rf_max),
            ]
        )
    else:
        return "\n".join(
...
def show_difftable_tab(difftable, extended=None):
    ...
    if extended:
        ...
        print(
            "#"
            + "\t".join(
                ["Dist", "Size1", "Size2", "ndiffs", "refTree", "targetTree", "Diff"]
            )
        )
...
def show_difftable_topo(difftable, attr1, attr2, usecolor=False, extended=None):
        ...
        if extended:
            showtable.append(
                [
                    dist,
                    b_dist,
                    "%d/%d (%d)" % (len(side1), len(side2), len(diff)),
                    topo1,
                    topo2,
                ]
            )
            ...
            showtable.append(
                [dist, "%d/%d (%d)" % (len(side1), len(side2), len(diff)), topo1, topo2]
            )

    if extended:
        print_table(
            showtable,
            header=["Dist", "branchDist", "#Diffs", "refTree", "targetTree"],
        ...
        print_table(
            showtable,
            header=["Dist", "#Diffs", "refTree", "targetTree"],
            max_col_width=maxcolwidth,
            wrap_style="wrap",
            row_line=True,
        )
    ...
    log.info(
        "Total euclidean distance:\t%0.4f\tMismatching nodes:\t%d"
...
def show_difftable_summary_SCA(difftable, rf=-1, rf_max=-1, extended=None):
    ...
    total_dist = 0
    total_bdist = 0
    for dist, b_dist, side1, side2, diff, n1, n2 in sorted(
        difftable, reverse=True, key=lambda x: x[0]
    ):
        total_dist += dist
        total_bdist += b_dist
...
def show_difftable_SCA(difftable, extended=False):
    ...
    if extended:
        ...
        print_table(
            showtable,
            header=["Dist", "branchDist", "Size1", "Size2", "refTree", "targetTree"],
            max_col_width=80,
            wrap_style="wrap",
            row_line=True,
        )
        ...
        for dist, b_dist, side1, side2, diff, n1, n2 in sorted(
            ...
            showtable.append(
                [dist, len(side1), len(side2), sepstring(side1), sepstring(side2)]
            )
        print_table(
            showtable,
            header=["Dist", "Size1", "Size2", "refTree", "targetTree"],
            max_col_width=80,
            wrap_style="wrap",
            row_line=True,
        )
...
def show_difftable_tab_SCA(difftable, extended=None):
    ...
    if extended:
        ...
        print(
            "#"
            + "\t".join(
                ["Dist", "branchDist", "Size1", "Size2", "refTree", "targetTree"]
            )
        )
    else:
        for dist, b_dist, side1, side2, diff, n1, n2 in sorted(
...
def show_difftable_topo_SCA(difftable, attr1, attr2, usecolor=False, extended=None):
    ...
    log.info(
        "Total distance:\t%0.4f\tMismatching nodes:\t%d" % (total_dist, len(difftable))
    )
...
def populate_args(diff_args_p):
    ...
    diff_args.add_argument(
        ...
        default="name",
        help=(
            "Defines the attribute in REFERENCE tree that will be used"
    ...
    diff_args.add_argument(
        ...
        default="name",
        help=(
            "Defines the attribute in TARGET tree that will be used"
            " to perform the comparison"
        ),
    )
    ...
    diff_args.add_argument(
        ...
        dest="distance",
        type=str,
        choices=["e", "rf", "eb", "eb-all", "eb-full"],
        default="e",
        help=(
            "Distance measure (e by default): e = Euclidean distance, rf = Robinson-Foulds symetric distance"
    ...
    diff_args.add_argument(
        ...
        action="store_false",
        help=(
            "Enable this option to if trivial results (distance 0) are not needed and duplicated attributes (i.e. name)"
    ...
    diff_args_r.add_argument(
        ...
        default=None,
        help=(
            "Extend with branch distance after node comparison (None by default). cc = Branch distance based in cophenetic-like distance between trees "
    ...
    diff_args_r.add_argument(
        ...
        type=str,
        help=(
            "For Single Cell Analysis data, csv/tsv (see --extension option) with cell IDs as columns headers and their "
    ...
    diff_args_r.add_argument(
        ...
        default="csv",
        help=(
            "Data separator for Single Cell Analysis data expression matrix (csv by default)"
        ),
    )
    ...
    diff_args_r.add_argument(
        ...
        default=None,
        help=(
            "Distance matrix computation parallelism (None by default). sync form syncronous and asyncc for asyncronous parallelism. "
    ...
    diff_args_r.add_argument(
        ...
        type=int,
        default=1,
        help="CAUTION: only for non linear parallelism (see --parallelism option). "
...
def run(args):
    ...
    if (not args.ref_trees or not args.src_trees) and (
        not args.rmatrix or not args.tmatrix
    ):
        logging.warning(
            "Target tree/matrix or reference tree/matrix weren't introduced. You can find the arguments in the help command (-h)"
        )
        ...
        if len(difftable) != 0:
            if dist_fn != SINGLECELL:
                if args.report == "topology":
                    show_difftable_topo(
                        difftable, rattr, tattr, usecolor=args.color, extended=extend
                    )
                ...
                if args.report == "topology":
                    ...
                    rf, rf_max, _, _, _, _, _ = t1.robinson_foulds(
                        t2, attr_t1="name", attr_t2="name"
                    )
                    show_difftable_summary_SCA(difftable, rf, rf_max, extended=extend)

        else:
            log.info(
                "Difference between (Reference) %s and (Target) %s returned no results"
                % (rtree, ttree)
            )
