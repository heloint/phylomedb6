...
def _get_expected_topology(t, species):
    ...
    if missing_sp:
        raise KeyError(
            "* The following species are not contained in the species tree: "
            + ",".join(missing_sp)
        )
...
def get_reconciled_tree_zmasek(gtree, sptree, inplace=False):
    ...
    # initialization
    sp2node = dict()
    for node in sptree.leaves():
        sp2node[node.species] = node
    ...
    for node in gtree.traverse(strategy="postorder"):
        ...
        lca = node.children[0].M.get_common_ancestor(
            node.children[1].M
        )  # LCA in the species tree
        node.add_prop("M", lca)
