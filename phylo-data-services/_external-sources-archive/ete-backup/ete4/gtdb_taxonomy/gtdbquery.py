...
class GTDBTaxa:
    ...
    def _translate_merged(self, all_taxids):
        ...
        cmd = (
            "select taxid_old, taxid_new FROM merged WHERE taxid_old IN (%s)"
    ...
    def _get_id2rank(self, internal_taxids):
        ...
        result = self.db.execute(
            "SELECT taxid, rank FROM species WHERE taxid IN (%s)" % ids
        )
    ...
    def _get_lineage_translator(self, taxids):
        ...
        result = self.db.execute(
            "SELECT taxid, track FROM species WHERE taxid IN (%s);" % query
        )
        id2lineages = {}
    ...
    def _get_name_translator(self, names):
        ...
        result = self.db.execute(
            "select spname, taxid from species where spname IN (%s)" % query
        )
        ...
        if missing:
            ...
            result = self.db.execute(
                "select spname, taxid from synonym where spname IN (%s)" % query
            )
            for sp, taxid in result.fetchall():
                oname = name2origname[sp.lower()]
                name2id.setdefault(oname, []).append(taxid)
    ...
    def get_descendant_taxa(
        ...
        _, conversion = self._translate_merged(
            [taxid]
        )  # try to find taxid in synonyms table
        if conversion:
        ...
        if rank_limit or collapse_subspecies or return_tree:
            ...
            tree = self.get_topology(
                list(descendants_spnames.values()),
            ...
            self._translate_to_names(
                [tid for tid, count in descendants.items() if count == 1]
            )
            return self._translate_to_names(
                [tid for tid, count in descendants.items() if count == 1]
            )
    ...
    def get_topology(
        ...
        tax2id = self._get_name_translator(
            taxnames
        )  # {'f__Korarchaeaceae': [2174], 'o__Peptococcales': [205487], 'p__Huberarchaeota': [610]}
        ...
        if len(taxids) == 1:
            ...
            all_taxid_codes = set(
                [_tax for _lin in list(id2lineage.values()) for _tax in _lin]
            )
            extra_tax2name = self._get_taxid_translator(
                list(all_taxid_codes - set(tax2name.keys()))
            )
        ...
        if not intermediate_nodes:
            for n in root.descendants():
                if (
                    len(n.children) == 1
                    and int(name2tax.get(n.name, n.name)) not in taxids
                ):
    ...
    def annotate_tree(
        ...
        all_taxid_codes = set(
            [_tax for _lin in list(tax2track.values()) for _tax in _lin]
        )
        extra_tax2name = self._get_taxid_translator(
            list(all_taxid_codes - set(tax2name.keys()))
        )
        ...
        for node in t.traverse("postorder"):
            ...
            if node_taxid:
                tmp_taxid = self._get_name_translator([node_taxid]).get(
                    node_taxid, [None]
                )[0]
                ...
                if rank != "subspecies":
                    ...
                    track = tax2track[
                        tmp_taxid
                    ]  # like ['root', 'd__Bacteria', ..., 's__Moorella', 'RS_GCF_0062.1']
                ...
                node.add_props(
                    ...
                    rank=tax2rank.get(tmp_taxid, "Unknown"),
                    named_lineage=[
                        tax2name.get(tax, str(tax))
                        for tax in tax2track.get(tmp_taxid, [])
                    ],
                )
            elif node.is_leaf:
                node.add_props(
                    sci_name=getattr(
                        node, taxid_attr, node.props.get(taxid_attr, "NA")
                    ),
                    common_name="",
                    lineage=[],
                    rank="Unknown",
                ...
                if ignore_unclassified:
                    vectors = [
                        lf.props.get("lineage")
                        for lf in n2leaves[node]
                        if lf.props.get("lineage")
                    ]
                else:
                ...
                if lineage[-1]:
                    if rank != "subspecies":
                        ancestor = self._get_taxid_translator([lineage[-1]])[
                            lineage[-1]
                        ]
                    else:
                        ancestor = self._get_taxid_translator([lineage[-2]])[
                            lineage[-2]
                        ]
                ...
                node.add_props(
                    sci_name=tax2name.get(ancestor, str(ancestor)),
...
if __name__ == "__main__":
    ...
    descendants = gtdb.get_descendant_taxa(
        "c__Thorarchaeia", collapse_subspecies=True, return_tree=True
    )
    ...
    tree = PhyloTree(
        "((c__Thorarchaeia, c__Lokiarchaeia_A), s__Caballeronia udeis);",
