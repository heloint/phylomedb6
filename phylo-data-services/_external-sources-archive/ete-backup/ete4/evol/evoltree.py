...
class EvolNode(PhyloTree):
    ...
    def _label_as_paml(self):
        ...
        if (len(self)) != len(set(self.leaf_names())):
            duplis = [n for n in self.leaf_names() if self.leaf_names().count(n) > 1]
    ...
    def get_descendant_by_node_id(self, idname):
        ...
        try:
            ...
            warn(
                "Should be first labelled as paml "
    ...
    def run_model(self, model_name, ctrl_string="", keep=True, **kwargs):
        ...
        **Note that M1 and M2 models are making reference to the new versions
        ...
        try:
            proc = Popen([bin_, "tmp.ctl"], stdout=PIPE, stdin=PIPE, stderr=PIPE)
        except OSError:
            raise Exception(
                ("ERROR: {} not installed, " + "or wrong path to binary\n").format(bin_)
            )
    ...
    run_model.__doc__ = run_model.__doc__ % (
        sep.join(
            [
                ...
                for x in sorted(
                    sorted(AVAIL.keys()), key=lambda x: AVAIL[x]["typ"], reverse=True
                )
            ]
        ),
    ...
    def link_to_alignment(
        ...
        """
        super(EvolTree, self).link_to_alignment(
            alignment, alg_format=alg_format, **kwargs
        )
        check_len = 0
    ...
    def mark_tree(self, node_ids, verbose=False, **kargs):
        ...
        for node in self.traverse():
            ...
            if node.props.get("node_id") in node_ids:
                if (
                    "." in marks[node_ids.index(node.props.get("node_id"))]
                    or match(
                        "#[0-9]+", marks[node_ids.index(node.props.get("node_id"))]
                    )
                    is None
                ) and verbose:
                    warn(
                ...
                node.add_prop(
                    "mark", " " + marks[node_ids.index(node.props.get("node_id"))]
                )
    ...
    def link_to_evol_model(self, path, model):
        ...
        while model.name in self._models:
            model.name = model.name.split("__")[0] + str(
                (int(model.name.split("__")[1]) + 1) if "__" in model.name else 0
            )
        ...
        try:
            if hasattr(altn, "lnL") and hasattr(null, "lnL"):
                if null.lnL - altn.lnL < 0:
                    return chi_high(
                        2 * abs(altn.lnL - null.lnL), float(altn.np - null.np)
                    )
                else:
                    warn(
            ...
            warn(
                "at least one of %s or %s, was not calculated" % (altn.name, null.name)
            )
