    ...
    for line in _source:
        ...
        # Reads seq number
            ...
            if (not fasta) and "   " in seq_header_fields[0].strip():
                ...
                if len_seq:
                    if len(SC.id2seq[seq_id]) == len_seq:
                        ...
                        raise Exception(
                            "Error reading sequences: Wrong sequence length.\n" + line
                        )
            else:
                SC.id2seq[seq_id] = ""
            ...
            if fix_duplicates and seq_name in names:
                tag = str(
                    len([k for k in list(SC.name2id.keys()) if k.endswith(seq_name)])
                )
                old_name = seq_name
                seq_name = tag + "_" + seq_name
                print(
                    "Duplicated entry [%s] was renamed to [%s]" % (old_name, seq_name),
                    file=STDERR,
                )
...
def write_paml(sequences, outfile=None, seqwidth=80):
    ...
    text += "\n".join(
        [
            "%s\n%s" % ("\t".join([name] + comment), _seq2str(seq))
            for name, seq, comment in sorted(sequences)
        ]
    )
