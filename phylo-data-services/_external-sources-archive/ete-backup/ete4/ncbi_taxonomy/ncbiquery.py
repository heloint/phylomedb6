...
class NCBITaxa:
    ...
    def _translate_merged(self, all_taxids):
        ...
        cmd = (
            "SELECT taxid_old, taxid_new "
            "FROM merged WHERE taxid_old IN (%s)" % ",".join(map(str, all_taxids))
        )
    ...
    def annotate_tree(
        ...
        extra_tax2name = self.get_taxid_translator(
            ...
            if node_taxid:
                ...
                n.add_props(
                    ...
                    named_lineage=[
                        ...
                        for tax in tax2track.get(node_taxid, [])
                    ],
                )
            elif n.is_leaf:
                n.add_props(
                    sci_name=getattr(n, taxid_attr, n.props.get(taxid_attr, "NA")),
                    common_name="",
                    lineage=[],
                    rank="Unknown",
                    named_lineage=[],
...
def upload_data(dbfile):
    ...
    for i, line in enumerate(open("syn.tab")):
        ...
        db.execute(
            "INSERT INTO synonym (taxid, spname) VALUES (?, ?);", (taxid, spname)
        )
    print()
    db.commit()
    for i, line in enumerate(open("merged.tab")):
        ...
        db.execute(
            "INSERT INTO merged (taxid_old, taxid_new) VALUES (?, ?);",
            (taxid_old, taxid_new),
        )
    print()
    db.commit()
...
if __name__ == "__main__":
    ...
    b = ncbi.get_descendant_taxa(
        "homo", intermediate_nodes=True, collapse_subspecies=True
    )
