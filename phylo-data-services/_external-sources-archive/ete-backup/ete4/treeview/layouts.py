...
def heatmap(node):
    ...
    ProfileFace = faces.ProfileFace(
        matrix_max, matrix_min, matrix_avg, square_size * ncols, square_size, "heatmap"
    )
...
def cluster_cbars(node):
    ...
    matrix_avg = matrix_min + ((matrix_max - matrix_min) / 2)
    ProfileFace = faces.ProfileFace(
        matrix_max, matrix_min, matrix_avg, 200, 60, "cbars"
    )
...
def cluster_lines(node):
    ...
    matrix_avg = matrix_min + ((matrix_max - matrix_min) / 2)
    ProfileFace = faces.ProfileFace(
        matrix_max, matrix_min, matrix_avg, 200, 50, "lines"
    )
...
def cluster_bars(node):
    ...
    if node.is_leaf:
        ...
        matrix_avg = matrix_min + ((matrix_max - matrix_min) / 2)
        ProfileFace = faces.ProfileFace(
            matrix_max, matrix_min, matrix_avg, 200, 40, "bars"
        )
        nameFace = faces.AttrFace("name", fsize=6)
        faces.add_face_to_node(nameFace, node, 1, aligned=True)
        faces.add_face_to_node(ProfileFace, node, 0, aligned=True)
...
def evol_layout(node):
    ...
    if hasattr(node, "extras"):
        faces.add_face_to_node(
            faces.AttrFace("extras", "Arial", 7, "#000000", None), node, 2
        )
    if hasattr(node, "sequence"):
        seqface = faces.SequenceFace(
            node.sequence, interactive=True, codon=node.nt_sequence
        )
...
def evol_clean_layout(node):
    ...
    if hasattr(node, "dN"):
        faces.add_face_to_node(
            faces.TextFace("%.4f" % (node.w), fsize=6, fgcolor="#7D2D2D"),
            node,
            0,
            position="branch-top",
        )
        faces.add_face_to_node(
            faces.TextFace(
                "%.2f/%.2f" % (100 * node.dN, 100 * node.dS), fsize=6, fgcolor="#787878"
            ),
            node,
            0,
