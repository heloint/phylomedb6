...
class _SelectorItem(QGraphicsRectItem):
    ...
    def paint(self, p, option, widget):
        ...
        if (
            ...
            and self.rect().height() > textR.height() / 2
            and 0
        ):  # OJO !!!!
...
class _GUI(QMainWindow):
    ...
    def on_actionFit2region_triggered(self):
        ...
        if R.width() > 0 and R.height() > 0:
            self.view.fitInView(
                R.x(), R.y(), R.width(), R.height(), Qt.AspectRatioMode.KeepAspectRatio
            )
        ...
        if ok:
            ...
            if mType >= 2 and mType <= 6:
                try:
                    ...
                    QMessageBox.information(self, "!", "A numeric value is expected")
                    return
            elif mType == 7:
            ...
            for n in self.scene.tree.traverse(is_leaf_fn=_leaf):
                if setup.leaves_only.isChecked() and not _leaf(n):
                    continue
                if hasattr(n, aName) and cmpFn(getattr(n, aName), aValue):
    ...
    def on_actionSave_newick_triggered(self):
        fname = QFileDialog.getSaveFileName(
            self, "Save File", "/home", "Newick (*.nh *.nhx *.nw )"
        )
    ...
    def on_actionRender_selected_region_triggered(self):
        if not self.scene.selector.isVisible():
            return QMessageBox.information(self, "!", "You must select a region first")
    ...
    def on_actionPaste_newick_triggered(self):
        text, ok = QInputDialog.getText(self, "Paste Newick", "Newick:")
        if ok:
            try:
                t = Tree(str(text))
            except Exception as e:
...
class _PropertiesDialog(QWidget):
        ...
        if hasattr(self.scene.tree, "_models"):
            ...
            if hasattr(next(self.scene.tree.leaves()), "nt_sequence"):
                ...
                self.combo_run.addItems(
                    ["%s (%s)" % (m, AVAIL[m]["typ"]) for m in avail_models]
                )
    ...
    def runModelButton(self):
        ...
        if AVAIL[model]["allow_mark"]:
            ...
            marks = [str(n.node_id) for n in self.scene.tree.descendants() if n.mark]
            if not marks:
    ...
    def get_props_in_nodes(self, nodes):
        ...
        for n in nodes:
            for pname, pvalue in n.props.items():
                if type(pvalue) == int or type(pvalue) == float or type(pvalue) == str:
...
class _TreeView(QGraphicsView):
    def __init__(self, *args):
        ...
        self.setRenderHints(
            QPainter.RenderHint.Antialiasing or QPainter.SmoothPixmapTransform
        )
        ...
        self.setRenderHints(
            ...
            or QPainter.RenderHint.SmoothPixmapTransform
        )
