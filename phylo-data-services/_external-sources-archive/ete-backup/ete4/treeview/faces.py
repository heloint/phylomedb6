...
class AttrFace(TextFace):
    ...
    def __init__(
        ...
        Face.__init__(self)
        TextFace.__init__(
            self, None, ftype, fsize, fgcolor, penwidth, fstyle, tight_text
        )
        self.attr = attr
        self.type = "text"
...
class ProfileFace(Face):
        ...
        for pos in range(vlength):
            ...
            if mean1 > self.center_v:
                color_index = abs(
                    int(
                        ceil(
                            ((self.center_v - mean1) * 100)
                            / (self.max_value - self.center_v)
                        )
                    )
                )
            ...
            p.fillRect(
                QRectF(x1 + 3, profile_height - mean_y1, x_alpha - 4, mean_y1 - 1),
                QBrush(customColor),
            )
    ...
    def draw_centered_bar_profile(self):
        ...
        x_alpha = float(profile_width / (len(mean_vector)))
        y_alpha_up = float(
            ((profile_height - 1) / 2) / (self.max_value - self.center_v)
        )
        y_alpha_down = float(
            ((profile_height - 1) / 2) / (self.min_value - self.center_v)
        )
        ...
        for pos in range(vlength):
            ...
            if mean1 < self.center_v:
                p.fillRect(
                    QRectF(x1 + 3, mean_line_y, x_alpha - 4, mean_y1),
                    QBrush(customColor),
                )
            else:
                p.fillRect(
                    QRectF(x1 + 3, mean_line_y - mean_y1, x_alpha - 4, mean_y1 + 1),
                    QBrush(customColor),
                )
    ...
    def draw_line_profile(self):
        ...
        for pos in range(0, vlength - 1):
            ...
            if dev1 != 0 and dev2 != 0:
                ...
                p.setPen(QColor("red"))
                p.drawLine(
                    QLineF(x1, profile_height - dev_y1, x2, profile_height - dev_y2)
                )
                p.drawLine(
                    QLineF(x1, profile_height + dev_y1, x2, profile_height + dev_y2)
                )
            ...
            p.drawLine(
                QLineF(x1, profile_height - mean_y1, x2, profile_height - mean_y2)
            )
...
class OLD_SequenceFace(Face):
    ...
    def __init__(
        self, seq, seqtype, fsize=10, aafg=None, aabg=None, ntfg=None, ntbg=None
    ):
...
def _label_painter(obj, p, option, widget):
    ...
    try:
        ...
        p.drawText(
            int(rect.width() / 2 - (metrics.width() / 2)),
...
class RectFace(Face):
    ...
    def update_items(self):
        self.item = _RectItem(
            self.width, self.height, self.bgcolor, self.fgcolor, label=self.label
        )
...
class CircleFace(Face):
    ...
    def update_items(self):
        if self.style == "circle":
            self.item = _SphereItem(
                self.radius, self.color, solid=True, label=self.label
            )
...
class PieChartFace(StaticItemFace):
    ...
    def update_items(self):
        self.item = _PieChartItem(
            self.percents, self.width, self.height, self.colors, self.line_color
        )
...
class _StackedBarItem(QGraphicsRectItem):
    ...
    def paint(self, painter, option, widget):
        ...
        for i, p in enumerate(self.percents):
            col = self.colors[i]
            w = (p * total_w) / 100.0  # assuming p is between 0 and 100
...
class _BarChartItem(QGraphicsRectItem):
    ...
    def set_real_size(self):
        ...
        if self.max_value is None:
            max_value = max(
                [v + d for v, d in zip(self.values, self.deviations) if isfinite(v)]
            )
        else:
        ...
        if self.draw_scale:
            ...
            max_string_metrics = fm.boundingRect(
                QRect(), Qt.AlignmentFlag.AlignLeft, max_string
            )
            min_string_metrics = fm.boundingRect(
                QRect(), Qt.AlignmentFlag.AlignLeft, min_string
            )
            scale_width = margin + max(
                max_string_metrics.width(), min_string_metrics.width()
            )
        ...
        if self.labels:
            ...
            label_height = (
                fm.boundingRect(
                    QRect(), Qt.AlignmentFlag.AlignLeft, longest_label
                ).width()
                + margin
            )
    ...
    def paint(self, p, option, widget):
        ...
        if self.draw_scale:
            ...
            p.drawLine(
                QLineF(
                    plot_width + margin - 1, 1, plot_width + margin - 1, plot_height + 1
                )
            )
        ...
        for pos in range(len(values)):
            ...
            if self.labels:
                ...
                p.rotate(90)
                p.drawText(
                    QRectF(0, -x_alpha, label_height, x_alpha),
            ...
            if std != 0:
                ...
                p.drawLine(
                    QLineF(
                        center_x,
                        plot_height - dev_up_y1,
                        center_x,
                        plot_height - dev_down_y1,
                    )
                )
                p.drawLine(
                    QLineF(
...
class SequenceItem(QGraphicsRectItem):
    def __init__(self, seq, seqtype="aa", poswidth=1, posheight=10, draw_text=False):
        QGraphicsRectItem.__init__(self)
        self.seq = seq
        ...
        for letter in self.seq:
            ...
            if x >= current_pixel:
                if self.draw_text and self.poswidth >= 8:
                    ...
                    p.drawText(
                        QRectF(x, 0, self.poswidth, self.posheight),
                    ...
                    p.drawLine(
                        QLineF(
                            x, self.posheight / 2, x + self.poswidth, self.posheight / 2
                        )
                    )
...
class SeqMotifFace(StaticItemFace):
    ...
    def update_items(self):
        ...
        max_h = max([reg[4] for index, reg in enumerate(self.regions)])
        y_center = max_h / 2
        ...
        for index, (seq_start, seq_end, typ, wf, h, fg, bg, name) in enumerate(
            self.regions
        ):
            ...
            if name and i:
                ...
                txt_item = TextLabelItem(
                    text, w, h, fsize=fsize, ffam=family, fcolor=fcolor
                )
...
class SequencePlotFace(StaticItemFace):
    ...
    def draw_y_axis(self):
        lineItem = QGraphicsLineItem(
            0, self.coordY(self.ylim[0]), 0, self.coordY(self.ylim[1]), parent=self.item
        )
        ...
        for y in set(self.hlines + list(self.ylim)):
            lineItem = QGraphicsLineItem(
                0, self.coordY(y), -5, self.coordY(y), parent=self.item
            )
            lineItem.setPen(QPen(QColor("black")))
            lineItem.setZValue(10)
    ...
    def draw_x_axis(self):
        ...
        for x in all_vals:
            ...
            # Center text according to masterItem size
            text.setPos(
                x * self.col_w - tw / 2 + self.col_w / 2, self.coordY(self.ylim[0]) + 6
            )
    ...
    def coordY(self, y):
        ...
        return (
            self.height
            + y_offset
            - (y - self.ylim[0]) / (self.ylim[1] - self.ylim[0]) * self.height
        )

    def draw_hlines(self, line, col):
        lineItem = QGraphicsLineItem(
            0, self.coordY(line), self.width, self.coordY(line), parent=self.item
        )
    ...
    def draw_bar(self, x, y, i):
        ...
        if y < self.ylim[1]:
            ...
            # upper left line
            lineItem = QGraphicsLineItem(
                0, coordY(y) - 4, 0, coordY(y) - 7, parent=item
            )
            lineItem.setX(x - 3)
            lineItem.setPen(QPen(QColor(self.colors[i]), 2))
            # upper right line
            ...
            # top line
            lineItem = QGraphicsLineItem(
                0, coordY(y) - 7, 6, coordY(y) - 7, parent=item
            )
            lineItem.setX(x - 3)
            lineItem.setPen(QPen(QColor(self.colors[i]), 2))
    ...
    def draw_stick(self, x, y, i):
        lineItem = QGraphicsLineItem(
            0, self.coordY(self.ylim[0]), 0, self.coordY(y), parent=self.item
        )
        lineItem.setX(x)
        lineItem.setPen(QPen(QColor(self.colors[i]), 2))
    ...
    def draw_errors(self, x, i):
        ...
        lineItem = QGraphicsLineItem(
            0, self.coordY(lower), 0, self.coordY(upper), parent=self.item
        )
        lineItem.setX(x)
    ...
    def draw_curve(self, x, y, i):
        # top line
        lineItem = QGraphicsLineItem(
            0, self.coordY(y), 4, self.coordY(y), parent=self.item
        )
        lineItem.setX(x - 2)
        lineItem.setPen(QPen(QColor(self.colors[i]), 2))
        if i > 0:
            ...
            lineItem = QGraphicsLineItem(
                0, self.coordY(prev), self.col_w - 4, self.coordY(y), parent=self.item
            )
...
class DiamondFace(StaticItemFace, Face):
    ...
    def update_items(self):
        self.item = _DiamondItem(
            width=self.width, height=self.height, label=self.label, color=self.color
        )
