...
def ascii_art(tree, show_internal=True, props=None, px=0, py=0, px0=0):
    ...
    descr = ",".join(
        (f"{k}={v}" for k, v in tree.props.items())
        if props is None
        else (str(tree.get_prop(p, "")) or "⊗" for p in props)
    )
...
def add_base(line, px, px0, txt, show_internal):
    ...
    return (
        (prefix_txt if show_internal else "")
        + "─" * px
        + replacements[line[padding]]
        + line[padding + 1 :]
    )
...
def get_branches_repr(are_last, is_leaf, px):
    ...
    prefix = "".join((" " if is_last else "│") + " " * px for is_last in are_last[:-1])

    return (
        prefix + ("└" if are_last[-1] else "├") + "─" * px + ("╴" if is_leaf else "┐")
    )
...
def to_repr(tree, depth=4, nchildren=3):
    ...
    children_repr = (
        ...
        if depth == 0
            ...
            + ("" if nchildren is None or len(tree.children) <= nchildren else ", ...")
        )
    )
