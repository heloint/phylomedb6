...
class LayoutGenomicContext(TreeLayout):
    def __init__(
        ...
        stroke_color="gray",
        stroke_width="1.5px",
        anchor_stroke_color="black",
        anchor_stroke_width="3px",
        non_conserved_color="#d0d0d0",
        collapse_conservation=0.2,
    ...
    def get_context(self, node):
        ...
        for pos, colors in sorted(color_context.items()):
            ...
            if (
                n / ntips >= self.collapse_conservation
                and color != self.non_conserved_color
            ):
                context.append(
                    {**color2genes[color], "vertical_conservation": n / ntips}
                )
            else:
