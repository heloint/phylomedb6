...
def get_layout_sciname():
    ...
    def layout_fn(node):
        if node.is_leaf:
            ...
            node.add_face(
                TextFace(sci_name, color="black", padding_x=2),
                column=0,
                position="branch_right",
            )

            node.add_face(
                TextFace(name_seq, color="grey", padding_x=2),
                column=1,
                position="branch_right",
            )
    ...
    def layout_fn(node):
        if node.props.get("gname"):
            ...
            if node.is_leaf:
                ...
                node.add_face(
                    gname_face, column=1, position="aligned", collapsed_only=True
                )
...
def get_layout_lca_rects():
    ...
    def layout_fn(node):
        ...
        if node.props.get("lca_node_name"):
            ...
            node.add_face(
                lca_face, position="aligned", column=level, collapsed_only=True
            )
