...
class LayoutSciName(TreeLayout):
    ...
    def set_node_style(self, node):
        if node.is_leaf:
            ...
            node.add_face(
                TextFace(sci_name, color=color, padding_x=2),
                column=0,
            ...
            node.add_face(
                TextFace(prot_id, color="Gray", padding_x=2),
                column=2,
                position="aligned",
            )
            ...
            for i, text in enumerate(texts):
                ...
                if text in sciName2color.keys():
                    ...
                    color = "black"
                node.add_face(
                    TextFace(text, padding_x=2, color=color),
                    position="branch_right",
                    column=1,
                    collapsed_only=True,
                )
    ...
    def set_node_style(self, node):
        if node.is_leaf:
            ...
            if target_leaf.props.get("Pname"):
                ...
                pname_face = TextFace(pname, color=self.text_color)
                node.add_face(
                    pname_face, column=3, position="aligned", collapsed_only=True
                )
...
class LayoutCuratedName(TreeLayout):
    ...
    def set_node_style(self, node):
        if node.is_leaf:
            if (
                node.props.get("curated_name")
                and node.props.get("curated_name") != "NA"
            ):
                ...
                spongCuratedName_face = TextFace(
                    spongCuratedName, color=self.text_color
                )
                node.add_face(spongCuratedName_face, column=5, position="aligned")
        else:
            target_leaf = node.get_leaves()[0]
            if (
                target_leaf.props.get("curated_name")
                and target_leaf.props.get("curated_name") != "NA"
            ):
                spongCuratedName = " ".join(
                    target_leaf.props.get("curated_name").split("_")
                )
                if len(spongCuratedName) > 30:
                    spongCuratedName = spongCuratedName[0:27] + " ..."
                spongCuratedName_face = TextFace(
    ...
    def set_node_style(self, node):
        ...
        if node.is_leaf:
            ...
            if target_leaf.props.get("OG_euk"):
                ...
                og_face = TextFace(OG, color=self.text_color)
                node.add_face(
                    og_face, column=6, position="aligned", collapsed_only=True
                )
