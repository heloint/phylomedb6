...
def interpolate_colors(c1, c2, mix=0):
    ...
    return "#" + "".join(
        "%02x"
        % int(
            (1 - mix) * int(c1[1 + 2 * i : 3 + 2 * i], 16)
            + mix * int(c2[1 + 2 * i : 3 + 2 * i], 16)
        )
        for i in range(3)
    )
            ...
            if len(unique):
                ...
                if self.legend:
                    tree_style.add_legend(
                        title=self.name, variable="discrete", colormap=self.colors
                    )
            else:
...
class LayoutBarplot(LayoutPlot):
    ...
    def set_node_style(self, node):
        ...
        if width and color:
            ...
            face = RectFace(
                width, None, color=color, tooltip=tooltip, padding_x=self.padding_x
            )
            node.add_face(
                face,
