...
class LayoutAlignment(TreeLayout):
    def __init__(
        ...
        self.length = (
            len(next(self.alignment.iter_entries())[1]) if self.alignment else None
        )
    ...
    def set_tree_style(self, tree, tree_style):
        if self.length:
            face = ScaleFace(
                width=self.width, scale_range=self.scale_range, padding_y=10
            )
    ...
    def set_node_style(self, node):
        ...
        if seq:
            seqFace = AlignmentFace(
                seq,
                seq_format=self.format,
                bgcolor="grey",
                width=self.width,
                height=self.height,
            )
