...
class Test_Core_ArrayTable(unittest.TestCase):
    ...
    def test_arraytable_parser(self):
        ...
        self.assertEqual(
            A.get_row_vector("A").tolist(),
            [-1.23, -0.81, 1.79, 0.78, -0.42, -0.69, 0.58],
        )
        self.assertEqual(
            A.get_several_row_vectors(["A", "C"]).tolist(),
            [
                [-1.23, -0.81, 1.79, 0.78, -0.42, -0.69, 0.58],
        ...
        self.assertEqual(
            A.get_several_column_vectors(["col2", "col7"]).tolist(),
            [
        ...
        self.assertEqual(
            A.get_column_vector("col4").tolist(),
        ...
        Abis = A.merge_columns(
            {"merged1": ["col1", "col2"], "merged2": ["col5", "col6"]}, "mean"
        )

        self.assertTrue(
            (
                np.round(Abis.get_column_vector("merged1"), 3)
