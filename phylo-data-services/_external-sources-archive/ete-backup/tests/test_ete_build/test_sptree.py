...
class Test_ete_build_sptree(unittest.TestCase):
    def test_01_sptree_worflow(self):
        ...
        for wkname in (
            "cog_all-alg_concat_default-raxml_default",
            "cog_all-alg_concat_default-fasttree",
        ):
            wkname = "cog_all-alg_concat_default-raxml_default"
            cmd = (
                "ete3 build -a %s/cog_seqs.fa --cogs %s/fake_cogs.tsv -w %s -m %s -o ete_test_tmp/etebuild_test2  -t0.5 --launch 0.5 --clearall --cpu %d"
                % (DIR, DIR, gene_wkname, wkname, CPUS)
            )
            args = cmd.split()
            ete._main(args)
            ctree, xtree, alg_used, alg, alg_trimmed, img, cmd = get_out_files(
                "ete_test_tmp/etebuild_test2", wkname, "cog_seqs.fa"
            )
            t1 = Tree(ctree)
            t2 = Tree(xtree)

            expected_tree = Tree(
                "(sp5:18.0858,((sp3:1.04727e-06,sp4:0.124439)0.49:3.18184,(sp2:1.04727e-06,sp1:1.04727e-06)0:1.04727e-06)1:18.0858);"
            )
