...
class Test_ete_build_manual_algs(unittest.TestCase):
            ...
            for _trimmer in trimmers:
                for _tester in testers:
                    for _builder in builders:
                        ...
                        cmd = (
                            "ete3 build -a %s/P53.alg.fa -w %s -o ete_test_tmp/etebuild_test2  -t0.3 --launch 0.5 --clearall --cpu %d"
                            % (DIR, wkname, CPUS)
                        )
                        args = cmd.split()
                        ete._main(args)
                        ctree, xtree, alg_used, alg, alg_trimmed, img, cmd = (
                            get_out_files(
                                "ete_test_tmp/etebuild_test2", wkname, "P53.alg.fa"
                            )
                        )
                        t1 = Tree(ctree)
                        t2 = Tree(xtree)
    ...
    def test_01_manual_alg_error(self):
        ...
        expected_tree = Tree(expected_nw)
        cmd = (
            "ete3 build -a %s/P53.fa -w %s -o ete_test_tmp/etebuild_test3  -t0.3 --launch 0.5 --clearall --cpu %d"
            % (DIR, wkname, CPUS)
        )
        args = cmd.split()
