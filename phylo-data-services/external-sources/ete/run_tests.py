...
tests = {
    ...
    "interactive": ["test_treeview/test_all_treeview.py"],
    "slow": ["slow/test_ncbiquery_force_download.py"],
}
...
def run_pytest(tfiles, verbose):
    os.system("cd tests; " + "pytest " + ("-v " if verbose else "") + " ".join(tfiles))
...
def get_args():
    ...
    add(
        ...
        "--list",
        action="store_true",
        help="list the tests in each category and exit",
    )
    add(
        "-i",
        "--include-interactive",
