            ...
            for _trimmer in trimmers:
                for _tester in testers:
                    for _builder in builders:
                        ...
                        cmd = (
                            "ete3 build -a %s/P53.fa -w %s -o ete_test_tmp/etebuild_test1  -t0.3 --launch 0.5 --clearall --cpu %d"
                            % (DIR, wkname, CPUS)
                        )
                        args = cmd.split()
                        ete._main(args)
                        ctree, xtree, alg_used, alg, alg_trimmed, img, cmd = (
                            get_out_files(
                                "ete_test_tmp/etebuild_test1", wkname, "P53.fa"
                            )
                        )
                        t1 = Tree(ctree)
                        t2 = Tree(xtree)
                        ...
                        if _tester != "none":
                            ...
                            if wkname.startswith("mafft_default-"):
                                self.assertEqual(
                                    wkname + t2.modeltester_bestmodel,
                                    wkname + "pmodeltest-HIVb+I+F!G",
                                )
                            else:
                                self.assertEqual(
                                    wkname + t2.modeltester_bestmodel,
                                    wkname + "pmodeltest-HIVb+G+F!I",
                                )
                        if _trimmer:
