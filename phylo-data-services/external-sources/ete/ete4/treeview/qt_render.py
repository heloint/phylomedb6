...
class _PointerItem(QGraphicsRectItem):
    ...
    def paint(self, p, option, widget):
        ...
        if (
            self.rect().width() > textR.width()
            and self.rect().height() > textR.height() / 2.0
            and 0
        ):  # OJO !!!!
...
def render(root_node, img, hide_root=False):
    ...
    if img.show_branch_length:
        bl_face = faces.AttrFace(
            "dist", fsize=8, ftype="Arial", fgcolor="black", formatter="%0.3g"
        )
    if img.show_branch_support:
        su_face = faces.AttrFace(
            "support", fsize=8, ftype="Arial", fgcolor="darkred", formatter="%0.3g"
        )
        ...
        if mode == "r":
            if img.optimal_scale_level == "full":
                ...
                farthest, dist = root_node.get_farthest_leaf(
                    topological=img.force_topology
                )
    ...
    if mode == "c":
        tree_radius, min_y, max_y, min_x, max_x = crender.render_circular(
            root_node, n2i, rot_step, img.pack_leaves
        )
    ...
    aligned_region_width = render_aligned_faces(
        img, mainRect, parent.tree_layer, n2i, n2f
    )
    ...
    if img.rotation:
        ...
        parent.setTransform(
            QTransform().translate(x, y).rotate(img.rotation).translate(-x, -y)
        )
    ...
    mainRect.adjust(
        -img.margin_left, -img.margin_top, img.margin_right, img.margin_bottom
    )
    ...
    if img.show_scale:
        if img.scale_length is None:
            ...
            if length > mainRect.width():
                ...
                raise ValueError(
                    "Custom scale bar length (TreeStyle.scale_length) is larger than the tree image"
...
def set_node_size(node, n2i, n2f, img):
    ...
    h2 = max(faceblock["branch-right"].h, aligned_height, min_separation)
    h = max(h1, h2)
    ...
    w = sum(
        [
            max(
                branch_length + node.img_style["size"],
...
def render_floatings(n2i, n2f, img, float_layer, float_behind_layer):
    ...
    for node, faces in n2f.items():
        face_set = [
            [float_layer, faces.get("float", None)],
        ...
        for parent_layer, fb in face_set:
            ...
            if img.mode == "c":
                ...
                start = (
                    item.branch_length + xtra - fb.w
                )  # if fb.w < item.branch_length else 0.0
...
def render_aligned_faces(img, mainRect, parent, n2i, n2f):
    ...
    # faces.
        ...
        img.mode == "c"
        and (img.scale or img._scale == 0)
        and not img.allow_face_overlap
    ):
    ...
    for node, fb in aligned_faces:
        ...
        if img.draw_guiding_lines and _leaf(node):
            ...
            guide_line = _LineItem(
                item.nodeRegion.width() - 1, item.center, x, item.center
            )
            pen = QPen()
                        ...
                        if isinstance(f, _TextFaceItem):
                            face_list.append(
                                [
                                    ...
                                    y2,
                                    nid,
                                    str(getattr(f, "face_label", f.text())),
                                ]
                            )
                            ...
                            face_list.append(
                                [x1, y1, x2, y2, nid, getattr(f, "face_label", None)]
                            )
        # nid += 1
