...
def pack_nodes(n2i):
    ...
    for node in sorted(n2i, key=lambda x: n2i[x].radius):  # Move closer items first
        ...
        if (
            node.is_leaf
            and item.extra_branch_line
            and item.extra_branch_line.line().dx() > 0
        ):
            ...
            for other_item in n2i.values():
                if item != other_item:
                    otherItemBoundingPoly = other_item.content.mapToScene(
                        other_item.nodeRegion
                    )
                    if itemBoundingPoly.intersects(otherItemBoundingPoly):
                        for part in itertools.chain(
                            other_item.static_items, other_item.movable_items
                        ):
            ...
            def move_node_towards_center(amount: int):
                ...
                item.extra_branch_line.setLine(
                    old_line.x1(),
                    old_line.y1(),
                    math.floor(old_line.x2() - amount),
                    old_line.y1(),
                )
            ...
            binary_search_for_position(
                0, math.floor(item.extra_branch_line.line().dx())
            )
...
def render_circular(root_node, n2i, rot_step, pack_leaves):
    ...
    for node in root_node.traverse("preorder", is_leaf_fn=_leaf):
        ...
        if not _leaf(node) and len(node.children) > 1:
            ...
            path.arcMoveTo(
                -start, -start, start * 2, start * 2, 360 - rot_start - rot_span
            )
            path.arcTo(
                -start,
                -start,
...
def calculate_optimal_scale(root_node, n2i, rot_step, img):
    ...
    for node in visited_nodes:
        ...
        if best_scale is None:
            best_scale = (
                (n2minradius[node] - n2sumwidth[node]) / ndist if ndist else 0.0
            )
        else:
            # Whats the expected radius of this node?
            current_rad = n2sumdist[node] * best_scale + (
                n2sumwidth[node] + root_opening
            )
            ...
            if current_rad < n2minradius[node]:
                ...
                if img.root_opening_factor:
                    best_scale = (n2minradius[node] - (n2sumwidth[node])) / (
                        n2sumdist[node] + (most_distant * img.root_opening_factor)
                    )
                    root_opening = most_distant * best_scale * img.root_opening_factor
                else:
                    best_scale = (
                        n2minradius[node] - (n2sumwidth[node]) + root_opening
                    ) / n2sumdist[node]
            ...
            if (
                img.optimal_scale_level == "full"
                and item.widths[1] > ndist * best_scale
            ):
    ...
    if not img.allow_face_overlap:
        ...
        for node in visited_nodes:
            if n2i[node].heights[5]:
                ...
                min_alg_scale = (
                    min(new_scale, min_alg_scale)
                    if min_alg_scale is not None
                    else new_scale
                )
