...
class NewickDialog(QDialog):
    ...
    def add_prop(self):
        ...
        if aName != "" and not self._conf.features_list.findItems(
            aName, Qt.MatchFlag.MatchCaseSensitive
        ):
...
class _NodeActions:
    ...
    def mouseDoubleClickEvent(self, e):
        if self.node:
            ...
            if item.highlighted:
                ...
                self.scene().view.highlight_node(
                    self.node,
                    fullRegion=True,
    ...
    def showActionPopup(self):
        ...
        if self.node.up is not None and self.scene().tree == self.node:
        ...
        if isinstance(self.node, EvolTree):
            ...
            if current_mark <= max_value:
                ...
                contextMenu.addAction(
                    "ETE-evol: mark node as " + mark, partial(self._gui_mark_node, mark)
                )
                contextMenu.addAction(
                    "ETE-evol: mark group as " + mark,
                    partial(self._gui_mark_group, mark),
                )

            if getattr(self.node, "mark", None):
                contextMenu.addAction(
                    "ETE-evol: clear mark in node", partial(self._gui_unmark_node)
                )
                contextMenu.addAction(
                    "ETE-evol: clear mark in group", partial(self._gui_unmark_group)
                )
    ...
    def add_children(self):
        n, ok = QInputDialog.getInt(
            None, "Add childs", "Number of childs to add:", 1, 1
        )
        if ok:
            for i in range(n):
    ...
    def populate_partition(self):
        n, ok = QInputDialog.getInt(
            None, "Populate partition", "Number of nodes to add:", 2, 1
        )
        if ok:
