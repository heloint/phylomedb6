...
class _FaceGroupItem(QGraphicsRectItem):  # I was about to name this FaceBookItem :)
    ...
    def set_min_column_widths(self, column_widths):
        ...
        self.columns = sorted(
            set(list(self.column2faces.keys()) + list(self.column_widths.keys()))
        )
    ...
    def update_columns_size(self, norender=False):
            ...
            for r, f in enumerate(faces):
                ...
                if f.rotation:
                    if f.rotation == 90 or f.rotation == 270:
                        ...
                        trans = lambda x, y: (
                            x0 + (x - x0) * cos(theta) + (y - y0) * sin(theta),
                            y0 - (x - x0) * sin(theta) + (y - y0) * cos(theta),
                        )
                        coords = (
                            trans(0, 0),
                            trans(0, height),
        ...
        if self.as_grid:
            self.h = max(
                [
                    sum([self.r2max_h[r] for r in rows.keys()])
                    for c, rows in self.sizes.items()
                ]
            )
        else:
    ...
    def rotate(self, rotation):
        ...
        for obj in self.childItems():
            if hasattr(obj, "rotable") and obj.rotable:
                ...
                obj.setTransform(
                    matrix.translate(x, y).rotate(rotation).translate(-x, -y)
                )
