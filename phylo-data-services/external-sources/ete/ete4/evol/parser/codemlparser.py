...
def parse_rst(path):
    ...
    for line in open(path):
        ...
        if typ is None and re.match(r"^[a-z]+.*(\d+\.\d{5} *){" + str(k) + "}", line):
...
def divide_data(pamout, model):
    ...
    for num in range(1, int(model.properties["params"]["ndata"])):
        ...
        for line in open(pamout):
            if copy == False and line.startswith("Data set " + str(num) + "\n"):
                copy = True
                continue
            if copy == True and line.startswith("Data set " + str(num + 1) + "\n"):
                break
            if copy == True:
                out.write(line)
        out.close()
        ...
        if model.typ == "site":
            ...
            for line in open(rst):
                if copy == False and re.match("\t" + str(num) + "\n", line) is not None:
                    copy = True
                    continue
                if (
                    copy == True
                    and re.match("\t" + str(num + 1) + "\n", line) is not None
                ):
                    copy = False
                if copy == True:
                    rstout.write(line)
            rstout.close()
            setattr(
                model, "data_" + str(num), parse_paml(pamout + "_" + str(num), model)
            )
        else:
            setattr(
                model, "data_" + str(num), parse_paml(pamout + "_" + str(num), model)
...
def parse_paml(pamout, model):
    ...
    for i, line in enumerate(all_lines):
        ...
        if line.startswith("kappa "):
            try:
                model.stats["kappa"] = float(re.sub(r".*(\d+\.\d+).*", "\\1", line))
            except ValueError:
...
def _check_paml_labels(tree, paml_labels, pamout, model):
    ...
    try:
        relations = sorted(
            [list(map(int, x.split(".."))) for x in paml_labels], key=lambda x: x[1]
        )
    except IndexError:
        return
    ...
    for rel in relations:
        try:
            ...
            if int(node.up.props.get("node_id")) != int(rel[0]):
                warn(
                    "WARNING: labelling does not correspond (bad tree?)!!\n"
                    + "         Getting them from "
                    + pamout
                )
...
def _get_labels_from_paml(tree, relations, pamout, model):
    ...
    for node in tree.traverse(strategy="postorder"):
        if node.is_root:
            continue
        paml_id = next(filter(lambda x: x[1] == node.props.get("node_id"), relations))[
            0
        ]
