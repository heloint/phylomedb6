...
class Model:
        ...
        for i, node in enumerate(self._tree.traverse()):
            if node.is_root:
                str_mark += str_line % (
                    ...
                    "None",
                    node.props.get("node_id"),
                    node.name or "ROOT",
                )
            else:
                str_mark += str_line % (
                    self.branches[node.props.get("node_id")]["mark"],
                    self.branches[node.props.get("node_id")].get("w", "None"),
                    node.props.get("node_id"),
                    node.name or "EDGE",
                )
        str_site = ""
    ...
    def __check_marks(self):
        ...
        for i, node in enumerate(self._tree.traverse()):
            if has_mark and self.properties["allow_mark"]:
                self.branches[node.props.get("node_id")] = {
                    "mark": node.props.get("mark") or " #0"
                }
        ...
        **kwargs
        ...
        if TREEVIEW:
            try:
                ...
                raise Exception(
                    "ERROR: no sites to display, only available "
    ...
    def get_ctrl_string(self, outfile=None):
        ...
        for prm in ["seqfile", "treefile", "outfile"]:
            string += "%15s%s%s\n" % (prm, sep, str(self.properties["params"][prm]))
        string += "\n"
        for prm in sorted(
            list(self.properties["params"].keys()),
            key=lambda x: sub("fix_", "", x.lower()),
        ):
    ...
    def colorize_rst(self, val, col=None):
        ...
        if not "site" in self.properties["typ"]:
            raise Exception(
                "ERROR: histogram are only for site and " "branch-site models."
            )
    ...
    def significance_by_site(self, val):
        ...
        """
        if not "site" in self.properties["typ"]:
            raise Exception("ERROR: only for site and " "branch-site models.")
        ps_model = "positive" in self.properties["evol"]
        categories = []
        for pval, curr_class in zip(self.sites[val]["pv"], self.sites[val]["class"]):
            if pval < 0.95:
