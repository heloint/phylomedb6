...
def populate_args(view_args_p):
    ...
    edit_gr.add_argument(
        ...
        action="store_true",
        help=(
            "Parses the newick string and extracts bootstrap values from"
    ...
    phylo_gr.add_argument(
        "--alg_type",
        dest="alg_type",
        choices=["blockseq", "compactseq", "fullseq"],
        default="blockseq",
    ...
    phylo_gr.add_argument(
        "--heatmap", dest="heatmap", type=str, help="""attr_name \t v1, v2, v3, v4"""
    )

    phylo_gr.add_argument(
        "--profile", dest="profile", type=str, help="""attr_name \t v1, v2, v3, v4"""
    )

    phylo_gr.add_argument("--bubbles", dest="bubbles", type=str, help="")
...
def run(args):
    if args.text_mode:
        for tindex, tfile in enumerate(src_tree_iterator(args)):
            ...
            print(
                t.to_str(
                    show_internal=args.show_internal_names, props=args.show_attributes
                )
            )
        return
    ...
    if args.heatmap:
        FACES.extend(parse_faces(["value:@name, size:10, pos:aligned, ftype:heatmap"]))
    ...
    for tindex, tfile in enumerate(src_tree_iterator(args)):
        ...
        if args.raxml:
            nw = re.sub(
                ":(\d+\.\d+)\[(\d+)\]", ":\\1[&&NHX:support=\\2]", open(tfile).read()
            )
        ...
        if args.heatmap:
            ...
            def gradient_color(value, max_value, saturation=0.5, hue=0.1):
                ...
                def hls2hex(h, l, s):
                    return rgb2hex(
                        tuple([int(x * 255) for x in colorsys.hls_to_rgb(h, l, s)])
                    )
        ...
        for node in t.traverse():
            ...
            for findex, f in enumerate(FACES):
                if (
                    f["nodetype"] == "any"
                    or (f["nodetype"] == "leaf" and node.is_leaf)
                    or (f["nodetype"] == "internal" and not node.is_leaf)
                ):
        ...
        for clause in map(str.strip, fargs.split(",")):
            ...
            if key == "if":
                ...
                if value != "italic" and value != "bold":
                    raise ValueError(
                        "valid style formats are: italic, bold [%s]" % clause
                    )
                face[key] = value
            elif key == "format":
                if "%" not in value:
                    print(value)
                    raise ValueError(
                        "format attribute should contain one format char: ie. %%s [%s]"
                        % clause
                    )
...
def maptrees_layout(node):
    ...
    if getattr(node, "maptrees_support", "NA") != "NA":
        f = CircleFace(
            radius=float(node.maptrees_support) / 10, color="blue", style="sphere"
        )
        f.opacity = 0.5
    ...
    if getattr(node, "maptrees_treeko_support", "NA") != "NA":
        node.add_face(f, column=1, position="float")
        node.add_face(
            AttrFace("maptrees_treeko_support"), column=1, position="branch-bottom"
        )
