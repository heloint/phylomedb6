...
def populate_args(compare_args_p):
    ...
    compare_args.add_argument(
        "--show_mismatches", dest="show_mismatches", action="store_true", help=""
    )

    compare_args.add_argument(
        "--show_matches", dest="show_matches", action="store_true", help=""
    )
    compare_args.add_argument(
        "--show_edges", dest="show_edges", action="store_true", help=""
    )
    ...
    for stree_name in src_tree_iterator(args):
        ...
        for rtree_name in ref_tree_iterator(args):
            ...
            if args.show_mismatches or args.show_matches or args.show_edges:
                ...
                if args.unrooted:
                    for tag, part in [
                        ("src: %s" % stree_name, src),
                        ("ref: %s" % rtree_name, ref),
                    ]:
                        print(
                            "%s\t%s"
                            % (
                ...
                data = [
                    shorten_str(stree_name, 15, reverse=True),
