...
class CogSelector(CogSelectorTask):
    def __init__(self, target_sp, out_sp, seqtype, conf, confname):
        ...
        CogSelectorTask.__init__(
            self, node_id, "cog_selector", "MCL-COGs", None, conf[confname]
        )
    ...
    def finish(self):
        ...
        for cognumber, seq_cogs in iter_cog_seqs(
            GLOBALS["cogs_file"], GLOBALS["spname_delimiter"]
        ):
        ...
        for sp, ncogs in sorted(
            list(sp2cogs.items()), key=lambda x: x[1], reverse=True
        ):
            log.log(
                28,
                "% 20s  found in single copy in  % 6d (%0.1f%%) COGs "
        ...
        log.log(
            28,
            "Largest cog size: %s. Smallest cog size: %s" % (largest_cog, smallest_cog),
        )
        self.cog_analysis = ""
        ...
        for co in self.raw_cogs:
            ...
            for sp, seq in co:
                sp_repr[sp] += 1
            co_names = [
                "%s%s%s" % (sp, GLOBALS["spname_delimiter"], seq) for sp, seq in co
            ]
        ...
        for sp, ncogs in sorted(
            list(sp_repr.items()), key=lambda x: x[1], reverse=True
        ):
            log.log(
                28,
                " % 30s species present in % 6d COGs (%0.1f%%)"
                % (sp, ncogs, 100 * ncogs / float(len(self.cogs))),
            )

        log.log(
            28,
            " %d COGs selected with at least %d species out of %d"
            % (len(self.cogs), min_species, len(all_species)),
        )
        log.log(
            28,
            " Average COG size %0.1f/%0.1f +- %0.1f"
        ...
        if missing_sp:
            log.error(
                "%d missing species or not present in single-copy in any cog:\n%s"
                % (len(missing_sp), "\n".join(missing_sp))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cogs_file", dest="cogs_file", required=True, help="Cogs file"
    )
    ...
    conf = {
        "user": {
            "_species_missing_factor": args.missing_factor,
