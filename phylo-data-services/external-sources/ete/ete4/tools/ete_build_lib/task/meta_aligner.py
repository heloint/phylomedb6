...
from ..utils import SeqGroup, OrderedDict, checksum, pjoin, GLOBALS, DATATYPES
...
def seq_reverser_job(multiseq_file, outfile, parent_ids, trimal_bin):
    ...
    job = Job(trimal_bin, reversion_args, "TrimalAlgReverser", parent_ids=parent_ids)
    return job
...
class MCoffee(AlgTask):
    def __init__(self, nodeid, seqtype, all_alg_files, conf, confname, parent_ids):
        ...
        self.conf = conf
        AlgTask.__init__(self, nodeid, "alg", "Mcoffee", base_args, self.conf[confname])
    ...
    def load_jobs(self):
        ...
        alg_paths = [pjoin(GLOBALS["input_dir"], algid) for algid in self.all_alg_files]
    ...
    def finish(self):
        ...
        alg_list_string = "\n".join(
            [pjoin(GLOBALS["input_dir"], aname) for aname in self.all_alg_files]
        )
...
class MetaAligner(AlgTask):
    def __init__(self, nodeid, multiseq_file, seqtype, conf, confname):
        ...
        # Initialize task
        AlgTask.__init__(
            self, nodeid, "alg", "Meta-Alg", OrderedDict(), self.conf[self.confname]
        )
    ...
    def load_jobs(self):
        ...
        first = seq_reverser_job(
            multiseq_file, multiseq_file_r, [self.nodeid], readal_bin
        )
        ...
        for aligner_name in self.conf[self.confname]["_aligners"]:
            ...
            # Normal alg
            task1 = _aligner(
                self.nodeid, self.multiseq_file, self.seqtype, self.conf, aligner_name
            )
            ...
            task3 = seq_reverser_job(
                reverse_out, reverse_out + "_restored", [task2.taskid], readal_bin
            )
