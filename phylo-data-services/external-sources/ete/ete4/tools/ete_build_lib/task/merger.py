...
class TreeMerger(TreeMergeTask):
    def __init__(self, nodeid, seqtype, task_tree, conf, confname):
        ...
        TreeMergeTask.__init__(
            self, nodeid, "treemerger", "TreeMerger", None, self.conf[self.confname]
        )
            ...
            if mainout.lower() == "midpoint":
                ...
                if mainout.startswith("~"):
                    ...
                    if len(outs) < 2:
                        raise TaskError(
                            self,
                            "First split outgroup error: common "
                ...
                if len(outs) > 1:
                    ...
                    if strict_common_ancestor and set(out_seqs) ^ outs:
                        msg = (
                            "Monophyly of first split outgroup could not be granted:%s"
                            % out_seqs
                        )
                        # dump_tree_debug(msg, self.taskdir, mtree, ttree, target_seqs, outs)
                        raise TaskError(self, msg)
                    ...
                    log.log(
                        26,
                        "@@8:First split rooting to %d seqs@@1:: %s"
                    ...
                    log.log(
                        26, "@@8:First split rooting to 1 seq@@1:: %s" % (single_out)
                    )
