...
class BrhCogCreator(CogSelectorTask):
    def __init__(self, target_sp, out_sp, seqtype, conf, confname):
        ...
        # Initialize task
        CogSelectorTask.__init__(
            self, node_id, "cog_selector", "Cog-Selector", None, conf[confname]
        )
    ...
    def finish(self):
        ...
        cogs, cog_analysis = brh_cogs2(
            db, all_species, missing_factor=self.missing_factor, seed_sp=self.seed
        )
        self.raw_cogs = cogs
...
def brh_cogs(DB, species, missing_factor=0.0, seed_sp=None, min_score=0):
    ...
    if seed_sp == "auto":
        ...
        log.log(
            28,
            "Using %s as search seed. Proteome size=%s genes"
    ...
    for j, seed in enumerate(sp_to_test):
        ...
        all_cogs = [
            cand for cand in list(cog_candidates.values()) if len(cand) >= min_species
        ]
...
def brh_cogs2(DB, species, missing_factor=0.0, seed_sp=None, min_score=0):
    ...
    if len(seqs & pre_selected_seqs) != len(set(seed2size[seed])) or len(
        seqs & pre_selected_seqs
    ) != len(seqs):
        print(
            "old method seqs",
            len(seqs),
            "new seqs",
            len(set(seed2size[seed])),
            "Common",
...
def get_sorted_seeds(seed, species, sp_to_test, min_species, DB):
    ...
    for j, seed in enumerate(sp_to_test):
        log.log(
            26,
            "Testing SIZE of new seed species:%s (%d/%d)",
            seed,
            j + 1,
            len(sp_to_test),
        )
        ...
        valid_seqs = [
            (k, len(v)) for k, v in counter.items() if len(v) >= min_species - 1
        ]
...
def get_best_selection(cogs_selections, species):
    ...
    def _compare_cog_selection(cs1, cs2):
        ...
        score_1, min_cov_1, max_cov_1, median_cov_1, cov_std_1, cog_cov_1 = (
            get_cog_score(
                candidates_1, sp2hits_1, median_cogs, ALL_SPECIES - set([seed_1])
            )
        )
        score_2, min_cov_2, max_cov_2, median_cov_2, cov_std_2, cog_cov_2 = (
            get_cog_score(
    ...
    for i, cogs in enumerate(cogs_selections):
        ...
        sp_percent_coverages = [
            (100 * sp2hits.get(sp, 0)) / float(len(candidates)) for sp in species
        ]
        ...
        data = (
            ...
            % (
                ...
                100 * float(len(ALL_SPECIES)) / (len(sp2hits) + 1),
            ),
            len(candidates),
            "%0.1f%% +- %0.1f"
            % (_mean(sp_percent_coverages), _std(sp_percent_coverages)),
            "% 3d (%0.1f%%)"
            % (min(sp_coverages), 100 * min(sp_coverages) / float(len(candidates))),
            "% 3d (%0.1f%%)"
            % (max(sp_coverages), 100 * max(sp_coverages) / float(len(candidates))),
            cog_cov,
            score,
        )
        if print_header:
            print_as_table(
                [data[1:]], header=header, print_header=True, stdout=cog_analysis
            )
            print_header = False
        else:
            print_as_table(
