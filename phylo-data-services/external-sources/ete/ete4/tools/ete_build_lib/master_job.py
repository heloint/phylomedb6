...
from .utils import md5, basename, pid_up, HOSTNAME, GLOBALS, TIME_FORMAT, pjoin
...
class Job:
    ...
    def __init__(self, bin, args, jobname=None, parent_ids=None):
        ...
        clean = lambda x: (
            basename(x) if GLOBALS["basedir"] in x or GLOBALS["tasks_dir"] in x else x
        )
        parsed_id_string = [
            "%s %s" % (clean(str(pair[0])), clean(str(pair[1])))
    ...
    def read_pid(self):
        try:
            host, pid = [
                _f.strip() for _f in open(self.pid_file, "r").readline().split("\t")
            ]
        except IOError:
            host, pid = "", ""
        else:
    ...
    def get_launch_cmd(self):
        return " ".join(
            [self.bin]
            + ["%s %s" % (k, v) for k, v in self.args.items() if v is not None]
        )
    ...
    def dump_script(self):
        ...
        lines = [
            "#!/bin/sh",
            " (echo R > %s && date +'%s' > %s) &&"
            % (self.status_file, TIME_FORMAT, self.time_file),
    ...
    def get_status(self, sge_jobs=None):
        ...
        if self.status not in set("DE"):
            ...
            if st == "E" and self.status is None:
                log.warning(
                    "@@3:Retrying job marked as error from previous executions.@@1:"
                )
                self.status = "W"
                try:
