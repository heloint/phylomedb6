...
def build_supermatrix_workflow(wname):
    try:
        cog_selector, alg_concatenator, treebuilder = map(
            lambda x: "@%s" % x, wname.split("-")
        )
    except ValueError:
...
def build_genetree_workflow(wname):
    try:
        aligner, trimmer, modeltester, treebuilder = map(
            lambda x: "none" if x.lower() == "none" else "@%s" % x, wname.split("-")
        )
    except ValueError:
...
def list_workflows(config, target_type=None):
    ...
    if not target_type or target_type == "supermatrix":
        print()
        avail_meta = [
            (k, config["workflow_desc"].get(k, ""), len(v))
            for k, v in config.get("supermatrix_meta_workflow", {}).items()
        ]
        print_table(
            avail_meta,
            fix_col_width=[45, 60, 10],
            header=["Worflow name", "Description", "threads"],
            title="Supermatrix shortcut workflow names",
            row_line=True,
        )
    ...
    if not target_type or target_type == "genetree":
        ...
        avail_meta = [
            (k, config["workflow_desc"].get(k, ""), len(v))
            for k, v in config.get("genetree_meta_workflow", {}).items()
        ]
        print_table(
            avail_meta,
            fix_col_width=[45, 60, 10],
            header=["Worflow name", "Description", "threads"],
            title="GeneTree shortcut workflow names",
            row_line=True,
        )
...
def list_apps(config, target_apps=None):
    for appname, validapps in APPTYPES.items():
        ...
        avail_blocks = [
            [blockname, block["_app"], block.get("_desc", "")]
            for blockname, block in config.items()
            if block.get("_app") in validapps
        ]
        print_table(
            avail_blocks,
            header=["name", "app type", "desc."],
            max_col_width=70,
            title=appname,
            row_line=True,
        )
...
def block_detail(block_name, config, color=True):
    ...
    if block_name not in config:
        try:
            next_block = [
                x.lstrip("@")
                for x in config.get("genetree_meta_workflow", {})[block_name]
            ]
            metaworkflow = True
    ...
    if metaworkflow and color:
        ...
        print(
            "%s = %s"
            % (block_name, ", ".join(config["genetree_meta_workflow"][block_name]))
        )
        print()
    elif metaworkflow:
...
def parse_block(blockname, conf):
    ...
    for tag, tester in CHECKERS.items():
        if tag[0] == blocktype and (tester[2] and tag[1] not in conf[blockname]):
            raise ConfigError(
                "[%s] attribute expected in block [%s]" % (tag[1], blockname)
            )
...
def check_config(fname):
    ...
    for k, v in list(conf.items()):
        if "_inherits" in v:
            ...
            try:
                ...
                raise ConfigError(
                    "[%s] config block is referred in [%s] but not present in config file"
                    % (base, k)
                )
            new_dict.update(v)
            conf[k] = new_dict
    ...
    for flow_name in conf.keys():
        ...
        npr_config = [
            len(v)
            for k, v in conf[flow_name].items()
            if type(v) == list and k != "target_levels"
        ]
        if len(set(npr_config)) != 1:
            raise ConfigError(
                "List values in [%s] should all have the same length" % flow_name
            )
    return conf
...
def check_block_link(conf, parent, v, attr_name):
    if isinstance(v, str) and v.startswith("@"):
        if v[1:] not in conf:
            raise ConfigError(
                "[%s] config block referred in [%s] but not found in config"
                % (v, parent)
            )
        if (
            attr_name in OPTION2APPTYPE
            and conf[v[1:]]["_app"] not in OPTION2APPTYPE[attr_name]
        ):
            raise ConfigError(
                "[%s] is not valid [%s] application block" % (v, attr_name)
            )
...
def check_number(value, cast, minv=0, maxv=None):
    try:
        ...
        if (minv is not None and typed_value < cast(minv)) or (
            ...
            raise ConfigError("[%s] not in the range (%s,%s)" % (value, _minv, _maxv))
    return typed_value
...
def is_appset_entry(value):
    if not isinstance(value, list) or len(value) != 2:
        raise ConfigError(
            "unexpected application format [%s]. Expected [appname, maxcpus] format"
            % value
        )
    try:
        cores = int(value[2])
        ...
        raise ConfigError(
            "unexpected application format [%s]. Expected [appname, maxcpus] format (maxcpus as integer)"
            % value
        )
...
def is_integer_or_percent(value):
    try:
        ...
        try:
            ...
            raise ConfigError(
                "[%s] should be an integer or a percentage (i.e. 15 or 0.4%%)" % value
            )
    return value
...
def is_raxml_bootstrap(value):
    try:
        ...
        if value == "alrt" or value == "alrt_phyml":
            ...
            raise ConfigError(
                '[%s] bootstrap value should an integer, "alrt" or "phyml_alrt"'
                % (value)
            )
...
CHECKERS = {
    ...
    ("cogselector", "_species_missing_factor"): (
        is_float,
        {"minv": 0, "maxv": 1},
        True,
    ),
