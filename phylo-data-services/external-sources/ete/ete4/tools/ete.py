...
def _main(arguments):
    ...
    if len(arguments) > 1:
        ...
        if subcommand == "version":
            ...
            if ete_path:
                builtin_apps_path = os.path.join(
                    os.path.split(ete_path)[0], "ete4_apps/bin"
                )
    ...
    # - MOD -
    mod_args_pp = subparser.add_parser(
        "mod",
        parents=[source_args_p, main_args_p, mod_args_p],
    ...
    # - EXTRACT -
    extract_args_pp = subparser.add_parser(
        "extract",
        parents=[source_args_p, main_args_p, extract_args_p],
    ...
    # - ANNOTATE -
    annotate_args_p = subparser.add_parser(
        "annotate",
    ...
    # - COMPARE -
    compare_args_p = subparser.add_parser(
        "compare",
        parents=[source_args_p, ref_args_p, main_args_p],
    ...
    # - VIEW -
    view_args_p = subparser.add_parser(
        "view",
        parents=[source_args_p, main_args_p],
    ...
    # - NCBIQUERY -
    ncbi_args_p = subparser.add_parser(
        "ncbiquery", parents=[main_args_p], description=ete_ncbiquery.DESC
    )
    ncbi_args_p.set_defaults(func=ete_ncbiquery.run)
    ete_ncbiquery.populate_args(ncbi_args_p)

    # - GENERATE -
    generate_args_p = subparser.add_parser(
        "generate",
        parents=[source_args_p, main_args_p],
    ...
    # - EVOL -
    evol_args_p = subparser.add_parser(
        "evol", parents=[source_args_p, main_args_p], description=ete_evol.DESC
    )
    evol_args_p.set_defaults(func=ete_evol.run)
    ete_evol.populate_args(evol_args_p)

    # - MAPTREES -
    maptrees_args_p = subparser.add_parser(
        "maptrees",
        parents=[source_args_p, ref_args_p, main_args_p],
    ...
    # - DIFF -
    diff_args_p = subparser.add_parser(
        "diff",
        parents=[source_args_p, ref_args_p, main_args_p],
    ...
    # - explore -
    explore_args_p = subparser.add_parser(
        "explore",
        parents=[source_args_p, main_args_p],
