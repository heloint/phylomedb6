...
def populate_args(maptrees_args_p):
    ...
    maptrees_args.add_argument(
        "--discard_incomplete",
        dest="discard_incomplete",
        action="store_true",
        help=("discard source trees missing any reference species"),
    )

    maptrees_args.add_argument(
        "--discard-root", dest="discard_root", action="store_true", help=("")
    )
        ...
        if not node.is_leaf:
            if len(node.children) != 2:
                if ignore_multifurcations:
                    ...
                    raise ValueError(
                        "multifurcations and single child branches not supported"
                    )

            b1 = set(
                [getattr(_c, target_attr) for _c in node2content[node.children[0]]]
            )
            b2 = set(
                [getattr(_c, target_attr) for _c in node2content[node.children[1]]]
            )
            if target_species:
...
def run(args):
    ...
    for rtree_nw in ref_tree_iterator(args):
        ...
        if args.ref_attr_parser:
            for leaf in rtree:
                leaf.add_property(
                    "_tempattr",
                    re.search(
                        args.ref_attr_parser, getattr(leaf, args.ref_tree_attr)
                    ).groups()[0],
                )
        ...
        reftree_species, reftree_branches = get_splits(
            rtree, target_attr=ref_tree_attr, ignore_multifurcations=True
        )
        ...
        for stree_count, stree_name in enumerate(src_tree_iterator(args)):
            ...
            if args.src_attr_parser:
                for leaf in stree:
                    leaf.add_property(
                        "name",
                        re.search(
                            args.src_attr_parser, getattr(leaf, args.src_tree_attr)
                        ).groups()[0],
                    )
                src_tree_attr = "name"
            ...
            srctree_species, srctree_branches = get_splits(
                stree,
                target_attr=src_tree_attr,
                discard_root=args.discard_root,
                target_species=reftree_species,
            )
            ...
            visited_refnodes = set(
                [
                    _rnode
                    for _rnode in rtree
                    if getattr(_rnode, ref_tree_attr) in srctree_species
                ]
            )
            ref2dups = defaultdict(int)
            for srcnode, refnode, isdup in map_dup_and_losses(
                rtree, srctree_branches, ref_leaves
            ):
            ...
            if args.treeko:
                ...
                for i, subnw in enumerate(sp_trees):
                    ...
                    treeko_srctree_species, treeko_srctree_branches = get_splits(
                        subtree, target_attr=src_tree_attr
                    )
                    for rbranch, matches in map_branches(
                        reftree_branches,
                ...
                # Count matches (flat)
                for rbranch, matches in map_branches(
                    reftree_branches, srctree_branches, reftree_species, srctree_species
                ):
        ...
        for node in rtree.traverse("levelorder"):
            ...
            observed = (
                float(len([s for s in supports if s > 0])) if has_support_data else "NA"
            )
