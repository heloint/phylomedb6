...
@get("/trees/<tree_id>/collapse_size")  # M
def callback(tree_id):
    tree_data, subtree = touch_and_get(tree_id)
    response.content_type = "application/json"
...
@get("/trees/<tree_id>/ultrametric")  # M
def callback(tree_id):
    tree_data, subtree = touch_and_get(tree_id)
    response.content_type = "application/json"
...
def callback(tree_id):
    ...
    assert delete_tabs(response_text) == delete_tabs(expected)

    # Verify the response code is 200
    assert response.status_code == 200

    expected: str = (
        '{"message": "when reading property <pname>: no node has the given property"}'
    )

    params = {"tree_id": 0, "pname": "tree_1"}
...
def run_smartview(
    ...
    # In the FastAPI implementation you can ignore this block. This is specific for the "Bottle" framework.
    # ==========================
