...
async function update() {
    ...
    if (view.minimap.show) update_minimap_visible_rect();
}
...
function get_tree_params() {
    ...
    const params_rect = {
        // parameters we have to pass to the drawer
        drawer: view.drawer.name,
        ...
        layouts: layouts,
        ultrametric: view.ultrametric ? 1 : 0,
    };

    const params_circ = {
        // parameters to the drawer, in circular mode
...
async function draw_tree() {
    ...
    try {
        ...
        const drawer_info = await api(
            `/drawers/${view.drawer.name}/${get_tid()}`
        );
        ...
        if (view.drawer.npanels > 1) {
            align_timeout = setTimeout(async () => {
                if (!align_drawing) await draw_aligned(params);
                clearTimeout(align_timeout);
...
function draw_tree_scale() {
    ...
    if (view.tree_scale.show) tree_scale.style.display = "block";
    else {
...
function draw(element, items, tl, zoom, replace = true) {
    ...
    if (html_container) {
        ...
        html_items.forEach((item) =>
            create_html(html_container, item, tl, zoom)
        );
    }
    ...
    if (replace) replace_svg(element);
...
function put_nodes_in_background(g) {
    ...
    Array.from(g.getElementsByClassName("box")).forEach((e) =>
        g.insertBefore(e, first)
    );
}

function replace_child(element, child) {
    if (element.children.length) element.children[0].replaceWith(child);
...
function update_aligned_panel_display() {
    ...
    if (view.drawer.type === "rect" && view.drawer.npanels > 1) {
        ...
        if (view.aligned.adjust_pos) {
            const tree_px_width =
                view.zoom.x * (view.tree_size.width - view.tl.x);
            view.aligned.pos =
                (100 * (tree_px_width + view.aligned.padding)) /
    ...
    } else div_aligned.style.display = "none"; // hide aligned panel
}

async function draw_legend(params) {
    if (!params) params = get_tree_params();
    ...
    if (items.length) {
        ...
        items.forEach((item) => {
            ...
            if (entry) div.appendChild(entry);
        });
        replace_child(div_legend, div);
        div_legend_container.style.display = "block";
    } else div_legend_container.style.display = "none";
}
...
async function draw_aligned(params, npanels) {
    ...
    const panels = {
        ...
        2: {
            div: div_aligned_header,
            show: view.aligned.header.show,
            params: {
                x: view.aligned.x,
                y: 0,
                w: div_aligned.offsetWidth / zx,
        ...
        3: {
            div: div_aligned_footer,
            show: view.aligned.footer.show,
            params: {
                x: view.aligned.x,
                y: 0,
                w: div_aligned.offsetWidth / zx,
                h: Math.max(
                    div_tree.offsetHeight / zy +
                        view.tl.y -
                        view.tree_size.height,
    ...
    if (view.drawer.type === "rect") {
        npanels = npanels
            ? Math.min(view.drawer.npanels, npanels)
            : view.drawer.npanels;
        for (let panel_n = 1; panel_n < npanels; panel_n++) {
            ...
            draw(
                div,
                items,
                { x: view.aligned.x, y: panel.params.y },
                { x: view.zoom.a, y: zy }
            );
...
function create_item(g, item, tl, zoom) {
    ...
    if (item[0] === "nodebox") {
        ...
        b.addEventListener("wheel", (event) => on_box_wheel(event, box), {
            passive: false,
        });
        b.addEventListener("mouseenter", (_) =>
            on_box_mouseenter(node_id, properties)
        );
        b.addEventListener("mouseleave", (_) => on_box_mouseleave(node_id));
        ...
        if (view.drawer.type == "circ") {
            const { x: cx, y: cy } = cartesian_shifted(
                center[0],
                center[1],
                tl,
                zx
            ...
            const angle =
                (Math.atan2(zy * tl.y + cy, zx * tl.x + cx) * 180) / Math.PI;
...
function create_continuous_legend_entry(entry) {
    function format_numbers(arr) {
        return arr.map((num) => {
            ...
            const rounded = num < 0.01 ? num.toExponential(0) : num.toFixed(3);
...
function create_asec(box, tl, z, type, style) {
    ...
    if (type) element.class = type;

    if (is_style_property(style.id)) element.id = style.id;

    return create_svg_element("path", element);
}

function cartesian_shifted(r, a, tl, z) {
    return { x: z * (r * Math.cos(a) - tl.x), y: z * (r * Math.sin(a) - tl.y) };
}
...
function create_outline(box, tl, zx, zy) {
    ...
    else return create_circ_outline(box, tl, zx);
}
...
function create_line(p1, p2, tl, zx, zy, type = "", parent_of = []) {
    ...
    const classes =
        ...
        parent_of
            .map((text) =>
                Object.keys(view.searches).includes(text)
                    ? get_search_class(text, "parents")
                    : get_selection_class(text, "parents")
            )
            .join(" ");
...
function create_circle(center, radius, tl, zx, zy, type = "") {
    ...
    else var { x, y } = cartesian_shifted(cx, cy, tl, zx);

    return create_svg_element("circle", {
        class: "circle " + type,
        cx: x,
        cy: y,
        r: radius, // view.node.dot.radius,
    });
}
...
function create_triangle(box, tip, tl, zx, zy, type = "") {
    const points = [];
    const [x, y, dx, dy] =
        view.drawer.type === "rect" ? transform_box(box, tl, zx, zy) : box;

    if (tip === "top")
        points.push([x + dx / 2, y], [x, y + dy], [x + dx, y + dy]);
    if (tip === "right") points.push([x, y], [x, y + dy], [x + dx, y + dy / 2]);
    if (tip === "bottom")
        points.push([x, y], [x + dx / 2, y + dy], [x + dx, y]);
    if (tip === "left")
        points.push([x, y + dy / 2], [x + dx, y + dy], [x + dx, y]);
...
function create_text(
    ...
    const element = {
        class: "text " + type,
        "text-anchor": style && style.text_anchor ? style.text_anchor : "left",
        "font-size": `${fs}px`,
    };
    ...
    } else t = create_svg_element("text", element);

    t.appendChild(document.createTextNode(text));

    if (view.drawer.type === "circ") {
        const angle =
            (Math.atan2(zy * tl.y + y, zx * tl.x + x) * 180) / Math.PI;
...
async function fix_text_orientations() {
    const texts = Array.from(div_tree.getElementsByClassName("text")).filter(
        is_upside_down
    );
...
function is_upside_down(element) {
    try {
        ...
        return false;
    } // texts in asec may cause exceptions
}
...
function addRotation(element, angle, cx = 0, cy = 0) {
    ...
    if (angle && -360 < angle < 360) tr.setRotate(angle, cx, cy);
...
function style_line(line, style) {
    if (is_style_property(style.type))
        line.style["stroke-dasharray"] =
            style.type === "solid" ? null : style.type == "dotted" ? 2 : 5;

    if (is_style_property(style["stroke-width"]))
        line.style["stroke-width"] = style["stroke-width"];

    if (is_style_property(style.stroke)) line.style.stroke = style.stroke;

    return line;
}

function style_outline(outline, style) {
    if (is_style_property(style.fill)) outline.style.fill = style.fill;

    if (is_style_property(style["fill-opacity"]))
        outline.style["fill-opacity"] = style["fill-opacity"];

    if (is_style_property(style.stroke)) outline.style.stroke = style.stroke;
...
function style_text(text, style) {
    if (is_style_property(style.fill)) text.style.fill = style.fill;

    if (is_style_property(style.ftype)) text.style["font-family"] = style.ftype;
...
function style_polygon(polygon, style) {
    if (is_style_property(style.fill)) polygon.style.fill = style.fill;

    if (is_style_property(style.opacity)) polygon.style.opacity = style.opacity;

    if (is_style_property(style.stroke)) polygon.style.stroke = style.stroke;
...
function style_html(html, style) {
    if (is_style_property(style.position)) html.style.position = style.position;

    if (is_style_property(style.overflow)) html.style.overflow = style.overflow;

    if (is_style_property(style.top)) html.style.top = style.top;

    if (is_style_property(style.bottom)) html.style.bottom = style.bottom;

    if (is_style_property(style.left)) html.style.left = style.left;

    if (is_style_property(style.right)) html.style.right = style.right;

    if (is_style_property(style.width)) html.style.width = style.width;

    if (is_style_property(style.height)) html.style.height = style.height;
}
