...
class LayoutLeafName(TreeLayout):
    def __init__(
        ...
        self.face = AttrFace(
            ...
            name="leaf_name",
            ftype=self.ftype,
            min_fsize=self.min_fsize,
            max_fsize=self.max_fsize,
            color=self.color,
            padding_x=self.padding.x,
            padding_y=self.padding.y,
        )
...
def _get_layout_branch_attr(
    ...
    def layout_fn(node):
        ...
        if (
            attr not in node.props  # missing property
            or node.dist == 0  # no space for drawing
            or node.is_leaf
            and attr == "support"
        ):  # should be 1
        ...
        node.add_face(branch_attr_face, position=pos, column=0, collapsed_only=True)
...
class LayoutBranchLength(TreeLayout):
    def __init__(
        ...
        self.set_node_style = _get_layout_branch_attr(
            attr="dist",
            formatter=formatter,
            name="Branch length",
            pos=pos,
...
class LayoutOutline(TreeLayout):
    def __init__(
        ...
        self.face = OutlineFace(
            stroke_color="black",
            stroke_width=stroke_width,
            color=color,
            opacity=opacity,
            collapsing_height=collapsing_height,
        )

    def set_node_style(self, node):
        if not node.is_leaf:
            node.add_face(
                self.face, position="branch_right", column=0, collapsed_only=True
            )


def get_layout_align_link(
    stroke_color="gray", stroke_width=0.5, line_type=1, opacity=0.8
):
    ...
    def layout_fn(node):
        if node.is_leaf:
            node.add_face(align_link_face, position="branch_right", column=1e9)
        else:
            node.add_face(
                align_link_face,
