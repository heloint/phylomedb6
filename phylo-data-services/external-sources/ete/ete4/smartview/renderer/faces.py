...
class Face:
    ...
    def compute_fsize(self, dx, dy, zx, zy, max_fsize=None):
        self._fsize = min(
            [dx * zx * CHAR_HEIGHT, abs(dy * zy), max_fsize or self.max_fsize]
        )
    ...
    def compute_bounding_box(
        ...
        if pos == "branch_top":  # above the branch
            ...
            avail_dy = dy / n_row
            aligned_x = (
                drawer.node_size(drawer.tree)[0] if drawer.panel == 0 else drawer.xmin
            )
    ...
    def _check_own_variables(self):
        if not self._box:
            raise Exception(
                f"**Box** has not been computed yet.\
...
class TextFace(Face):
    ...
    def draw(self, drawer):
        ...
        yield draw_text(
            self._box, self._content, self.name, rotation=self.rotation, style=style
        )
...
class AttrFace(TextFace):
    ...
    def _check_own_node(self):
        if not self.node:
            raise Exception(
                f"An associated **node** must be provided to compute **content**."
            )

    def get_content(self):
        content = str(
            getattr(self.node, self._attr, None) or self.node.props.get(self._attr)
        )
...
class CircleFace(Face):
    ...
    def __init__(self, radius, color, name="", tooltip=None, padding_x=0, padding_y=0):
    ...
    def draw(self, drawer):
        ...
        yield draw_circle(
            self._center, self._max_radius, self.name, style=style, tooltip=self.tooltip
        )
...
class RectFace(Face):
    ...
    def compute_bounding_box(
        ...
        zx = (
            1
            if self.stretch and pos.startswith("aligned") and drawer.TYPE != "circ"
            else zx
        )
    ...
    def draw(self, drawer):
        ...
        yield draw_rect(self._box, self.name, style=style, tooltip=self.tooltip)
        ...
        if self.text:
            ...
            if self.rotate_text:
                rotation = 90
                self.compute_fsize(
                    dy * zy / (len(self.text) * zx) * r, dx * zx / zy, zx, zy
                )

                text_box = Box(
                    x + (dx - self._fsize / (2 * zx)) / 2, y + dy / 2, dx, dy
                )
            else:
                rotation = 0
                self.compute_fsize(dx / len(self.text), dy, zx, zy)
                text_box = Box(
                    x + dx / 2, y + (dy - self._fsize / (zy * r)) / 2, dx, dy
                )
            text_style = {
                "max_fsize": self._fsize,
                "text_anchor": "middle",
...
class ArrowFace(RectFace):
    ...
    def orientation(self, value):
        if value not in ("right", "left"):
            raise InvalidUsage(
                'Wrong ArrowFace orientation {value}. Set value to "right" or "left"'
            )
        else:
...
class SelectedCircleFace(SelectedFace, CircleFace):
    def __init__(self, name, radius=15, padding_x=0, padding_y=0):
...
class SelectedRectFace(SelectedFace, RectFace):
    def __init__(self, name, width=15, height=15, text=None, padding_x=1, padding_y=0):
...
class OutlineFace(Face):
    ...
    def compute_bounding_box(
        ...
        self.outline = (
            drawer.outline
            if drawer.outline and len(drawer.outline) == 4
            else Box(0, 0, 0, 0)
        )
...
class AlignLinkFace(Face):
    def __init__(self, stroke_color="gray", stroke_width=0.5, line_type=1, opacity=0.8):
    ...
    def draw(self, drawer):
        ...
        if (
            drawer.panel == 0
            and drawer.viewport
            and (self.node.is_leaf or self.node.is_collapsed)
            and self.line
        ):
            p1, p2 = self.line
    ...
    def build_regions(self):
        ...
        if not motifs:
            if self.seq_format == "seq":
                motifs = [[0, len(seq), "seq", 15, self.height, None, None, None]]
            else:
                motifs = []
                pos = 0
    ...
    def draw(self, drawer):
        ...
        if self.gap_format in ["line", "-"]:
            ...
            yield draw_line(
                p1,
                p2,
                style={"stroke-width": self.gap_linewidth, "stroke": self.gapcolor},
            )
...
class ScaleFace(Face):
    ...
    def draw(self, drawer):
        ...
        if drawer.TYPE == "circ":
            ...
            p2 = cartesian(p2)
        yield draw_line(
            p1, p2, style={"stroke-width": self.line_width, "stroke": self.color}
        )
        ...
        if self.viewport:
            ...
            sm_end = nticks - round(
                max(x0 + self.width - (self.viewport[1] + self.viewport_margin), 0) / dx
            )
        else:
...
class ImgFace(RectFace):
    def __init__(self, img_path, width, height, name="", padding_x=0, padding_y=0):
        ...
        if extension not in ALLOWED_IMG_EXTENSIONS:
            print(
                "The image does not have an allowed format: "
                + extension
                + " not in "
                + str(ALLOWED_IMG_EXTENSIONS)
            )
...
class LegendFace(Face):
    ...
    def __init__(
        ...
        Face.__init__(self, name=title, padding_x=padding_x, padding_y=padding_y)
    ...
    def draw(self, drawer):
        ...
        yield draw_text(title_box, self.title, style=text_style)

        entry_y = y + 2 * entry_h
        for value, color in self.colormap.items():
            text_box = Box(x, entry_y, dx, entry_h)
            yield draw_text(text_box, value, style=text_style)
            ty += entry_h
...
class StackedBarFace(RectFace):
    ...
    def draw(self, drawer):
        ...
        for _, value, color, *_ in self.data:
            ...
            box = Box(x, y0, dx, dy0)
            yield draw_rect(box, self.name, style={"fill": color}, tooltip=self.tooltip)
            x += dx
