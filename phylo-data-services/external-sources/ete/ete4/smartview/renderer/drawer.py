...
class Drawer:
    ...
    def on_last_visit(self, point, it, graphics):
        ...
        # Searches
        searched_by = set(
            text for text, (results, _) in self.searches.items() if it.node in results
        )
        # Selection
        selected_by = [
            text for text, (results, _) in self.selected.items() if it.node in results
        ]
        active_clade = (
            ["active_clades"] if it.node in self.active.clades.results else []
        )
        ...
        if self.outline:
            if all(child in self.collapsed for child in it.node.children):
                searched_by.update(
                    text
                    for text, (results, parents) in self.searches.items()
                    if any(
                        node in results or node in parents.keys()
        ...
        content_graphics = list(
            self.draw_content(
                it.node, (x_before, y_before), active_children, selected_children
            )
        )
        graphics += content_graphics

        ndx = (
            drawn_size(content_graphics, self.get_box).dx
            if it.node.is_leaf
        ...
        self.nodeboxes += self.draw_nodebox(
            it.node,
            it.node_id,
            box,
    ...
    def draw_content(
        self, node, point, active_children=TreeActive(0, 0), selected_children=[]
    ):
        ...
        if not node.is_collapsed or node.is_leaf:
            ...
            yield from self.draw_node(
                node, point, dx, bdy, bdy0_, bdy1_, active_children, selected_children
            )
        ...
        if self.panel == 0:
            ...
            if dx > 0:
                parent_of = set(
                    text
                    for text, (_, parents) in self.searches.items()
                    if node in parents.keys()
                )
                parent_of.update(
                    text
                    for text, (_, parents) in self.selected.items()
                    if node in parents.keys()
                )
                hz_line_style = {
                ...
                yield from self.draw_lengthline(
                    (x, y + bdy),
                    (x + dx, y + bdy),
                    list(parent_of),
                    style=hz_line_style,
            ...
            if bdy0 != bdy1:
                ...
                yield from self.draw_childrenline(
                    (x + dx, y + bdy0), (x + dx, y + bdy1), style=vt_line_style
                )

            active_node = (
                "selected_results_active_nodes"
                if node in self.active.nodes.results
                else ""
            )
            nodedot_style = {
            ...
            yield from self.draw_nodedot(
                (x + dx, y + bdy), dy * self.zoom[1], active_node, nodedot_style
            )
    ...
    def draw_aligned_headers(self):
        ...
        def draw_faces_at_pos(faces, pos, iteration):
            ...
            for col, face_list in sorted(faces.items()):
                ...
                for row, face in enumerate(face_list):
                    drawn_face = list(
                        draw_face(face, pos, row, n_row, n_col, dx_before, dy_before)
                    )
                    if drawn_face:
        ...
        if self.panel == 2:
            deque(
                draw_faces_at_pos(
                    self.tree_style.aligned_panel_header, "aligned_bottom", 0
                )
            )
            graphics += draw_faces_at_pos(
                self.tree_style.aligned_panel_header, "aligned_bottom", 1
            )

        if self.panel == 3:
            deque(
                draw_faces_at_pos(
                    self.tree_style.aligned_panel_footer, "aligned_top", 0
                )
            )
            graphics += draw_faces_at_pos(
                self.tree_style.aligned_panel_footer, "aligned_top", 1
            )
    ...
    def get_outline(self):
        ...
        active_clade = (
            ["active_clades"] if collapsed_node in self.active.clades.results else []
        )
        active_children = self.get_active_children()
        selected_children = self.get_selected_children()
        ...
        if uncollapse:
            ...
            graphics += self.draw_collapsed(
                collapsed_node, active_children, selected_children
            )

        is_manually_collapsed = collapsed_node in self.collapsed
        is_small = self.is_small(make_box((x, y), self.node_size(collapsed_node)))
        ...
        if is_manually_collapsed or is_small or dist(collapsed_node) == 0:
            ...
            box = dh.draw_nodebox(
                self.flush_outline(ndx),
                name,
                properties,
                node_id,
                searched_by + selected_by + active_clade,
    ...
    def get_collapsed_node(self):
        ...
        parent = node0.up
        if all(node.up == parent for node in self.collapsed[1:]) and all(
            child in self.collapsed for child in parent.children
        ):
    ...
    def get_active_children(self):
        ...
        clades = sum(
            len(node) for node in self.collapsed if node in self.active.clades.results
        )
        clades += sum(
            self.active.clades.parents.get(node, 0) for node in self.collapsed
        )
        return TreeActive(nodes, clades)
    ...
    def get_popup_props(self, node):
        ...
        include_props = (
            self.include_props if self.include_props is not None else node.props
        )

        return {
            k: safe_string(node.props[k])
            for k in include_props
    ...
    def draw_collapsed(
        self, collapsed_node, active_children=TreeActive(0, 0), selected_children=[]
    ):
    ...
    def draw_nodedot(self, center, max_size, active_node, style):
        ...
        if size > 0:
            ...
            if style["shape"] == "circle":
                yield dh.draw_circle(
                    center,
                    radius=size,
                    circle_type="nodedot " + active_node,
                    style=nodedot_style,
                )
                ...
                yield dh.draw_rect(
                    box, rect_type="nodedot " + active_node, style=nodedot_style
                )
                ...
                dx, dy = 2 * size / zx, 2 * size / zy
                box = (x - dx / 2, y - dy / 2, dx, dy)
                yield dh.draw_triangle(
                    box,
                    "top",
                    triangle_type="nodedot " + active_node,
                    style=nodedot_style,
                )

    def draw_nodebox(self, node, node_id, box, searched_by, style=None):
        yield dh.draw_nodebox(
            box, node.name, self.get_popup_props(node), node_id, searched_by, style
        )
...
class DrawerCirc(Drawer):
    ...
    def in_viewport(self, box, pos=None):
        ...
        if self.panel == 0 and pos != "aligned":
            return dh.intersects_box(
                self.viewport, dh.circumrect(box)
            ) and dh.intersects_segment((-pi, +pi), dh.get_ys(box))
        else:
    ...
    def draw_lengthline(self, p1, p2, parent_of, style):
        ...
        if -pi <= p1[1] < pi:  # NOTE: the angles p1[1] and p2[1] are equal
            yield dh.draw_line(
                dh.cartesian(p1), dh.cartesian(p2), "lengthline", parent_of, style
            )
...
class DrawerRectFaces(DrawerRect):
    ...
    def draw_node(
        ...
        dx_to_closest_child = (
            dist(node) if node.is_leaf else min(dist(child) for child in node.children)
        )
        zx, zy, za = self.zoom
        ...
        def draw_faces_at_pos(node, pos):
            ...
            if pos == self.tree_style.selected_face_pos and len(selected_children):
                faces[n_col] = [
                    self.tree_style.selected_face(s, text=v)
                    for s, v in selected_children
                ]
                n_col += 1
            ...
            if pos == self.tree_style.active_face_pos:
                ...
                if nodes > 0:
                    active_faces.append(
                        self.tree_style.active_face("active_nodes", text=nodes)
                    )
                clades = active_children.clades
                if clades > 0:
                    active_faces.append(
                        self.tree_style.active_face("active_clades", text=clades)
                    )
                if active_faces:
            ...
            for col, face_list in sorted(faces.items()):
                ...
                for row, face in enumerate(face_list):
                    ...
                    if drawn_face:
                        ...
                        hfaces = self.tree_style._aligned_panel_header.get(
                            col
                        )  # header faces
                        ...
                        if col > 0 and hfaces:
                            dx_max = (
                                max(
                                    (hface.width for hface in hfaces if hface.width),
                                    default=0,
                                )
                                + hz_padding
                            )  # it's possible width is none
...
class DrawerCircFaces(DrawerCirc):
    ...
    def draw_node(
        ...
        dr_to_closest_child = (
            ...
            if not (node.is_leaf or node.is_collapsed)
            else dist(node)
        )
        z = self.zoom[0]  # zx == zy
        ...
        def draw_faces_at_pos(node, pos):
            ...
            if pos == self.tree_style.selected_face_pos and len(selected_children):
                faces[n_col] = [
                    self.tree_style.selected_face(s) for s in selected_children
                ]
                n_col += 1
            ...
            for col, face_list in sorted(faces.items()):
                ...
                for row, face in enumerate(face_list):
                    face.node = node
                    drawn_face = list(
                        draw_face(face, pos, row, n_row, n_col, dr_before, da_before)
                    )
                    if drawn_face:
...
def get_drawers():
    return [
        ...
        DrawerRectFaces,
        DrawerCircFaces,
        DrawerAlignRectFaces,
        DrawerAlignCircFaces,
    ]
