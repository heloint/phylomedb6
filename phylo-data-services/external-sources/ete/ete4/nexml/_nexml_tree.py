...
class NexmlTree(PhyloTree):
    ...
    def _set_children(self, value):
        if (
            isinstance(value, Children)
            and len(set([type(n) == type(self) for n in value])) < 2
        ):
    ...
    def __init__(
        ...
        self._nexml_support = LiteralMeta(
            datatype="float", property="branch_support", content=1.0
        )
    ...
    def export(
        ...
        if self.nexml_tree:
            ...
            self.nexml_tree.export(
                outfile=outfile, level=level, name_=name_, namespacedef_=namespacedef_
            )
