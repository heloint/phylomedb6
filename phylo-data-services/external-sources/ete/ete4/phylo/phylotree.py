...
def get_subparts(n):
    ...
    if is_dup(n):
        ...
        freaks = [
            _n
            for _n in n.descendants()
            if len(_n.children) == 1 or (not hasattr(_n, "_leaf") and not _n.children)
        ]
        for s in freaks:
...
class PhyloTree(Tree):
    ...
    def species(self):
        if self.props.get("_speciesFunction"):
            if "species" in self.props:
                warnings.warn(
                    "Ambiguous species: both species and _speciesFunction"
    ...
    def get_age_balanced_outgroup(self, species2age):
        ...
        for leaf in root.descendants():
            ...
            out_species = [
                self.props.get("_speciesFunction")(s) for s in all_seqs - leaf_seqs
            ]
    ...
    def collapse_lineage_specific_expansions(self, species=None, return_copy=True):
        ...
        if species and isinstance(species, (list, tuple)):
            ...
            raise TypeError(
                "species argument should be a set (preferred), list or tuple"
            )
        ...
        is_expansion = lambda n: (
            len(n2sp[n]) == 1
            and len(n2leaves[n]) > 1
