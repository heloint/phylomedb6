...
# gradients.
matrix_dist = [
    i for r in xrange(len(array.matrix)) for i in array.matrix[r] if numpy.isfinite(i)
]
...
def mylayout(node):
    ...
    # If node is internal
        ...
        if node.silhouette > 0:
            validationFace = TextFace(
                "Silh=%0.2f" % node.silhouette, "Verdana", 10, "#056600"
            )
            node.img_style["fgcolor"] = "#056600"
        # Otherwise, use red bubbles
        else:
            validationFace = TextFace(
                "Silh=%0.2f" % node.silhouette, "Verdana", 10, "#940000"
            )
