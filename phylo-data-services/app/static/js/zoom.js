...
function zoom_into_box(box, border = 0.1) {
...
function zoom_around(
    point,
    zoom_in,
    do_zoom = { x: true, y: true },
    qz = undefined,
) {
    tooltip.style.display = "none";
    if (!qz && zoom_in) {
        const dx = view.zoom_box[2];
        const dy = view.zoom_box[3];
        const qx = (0.8 * div_tree.offsetWidth) / (dx * view.zoom.x) - 1;
        const qy = (0.8 * div_tree.offsetHeight) / (dy * view.zoom.y) - 1;
        qz = {
            x: 1 + view.zoom.delta.in * Math.atan(qx),
            y: 1 + view.zoom.delta.in * Math.atan(qy),
            a: 1 + view.zoom.delta.in,
        };
    } else if (!qz && !zoom_in) {
        let zoomOutScaleRatio;
        if (view.drawer.type === "rect") {
            zoomOutScaleRatio = 3;
        } else if (view.drawer.type === "circ") {
            zoomOutScaleRatio = 10;
        }
        const dx = zoomOutScaleRatio * view.tree_size.width;
        const dy = zoomOutScaleRatio * view.tree_size.height;
        const qx = div_tree.offsetWidth / (dx * view.zoom.x) - 1;
        const qy = div_tree.offsetHeight / (dy * view.zoom.y) - 1;
        qz = {
            x: 1 + view.zoom.delta.in * Math.atan(qx),
            y: 1 + view.zoom.delta.in * Math.atan(qy),
            a: 1 + view.zoom.delta.out,
        };
    }
...
function zoom_xy(point, qz, do_zoom) {
    ...
    if (do_zoom.x || do_zoom.y) smooth_zoom(point);
}
...
function zoom_angular(point, qz) {
    ...
    if (zooming.timeout) window.clearTimeout(zooming.timeout);
...
function zoom_aligned(point, zoom_in) {
    ...
    point.x -= x0;
    const qz = {
        a: 1 + (zoom_in ? view.zoom.delta.in * 0.5 : view.zoom.delta.out * 0.5),
    };
    ...
    if (view.aligned.adjust_zoom) {
        ...
        zoom_new = view.aligned.max_zoom
            ? Math.min(view.aligned.max_zoom, zoom_new)
            : zoom_new;
    }
