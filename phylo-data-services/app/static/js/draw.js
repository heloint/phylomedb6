...
/**
* This is a path that fixes the arrow faces of the gene
* order feature in the implementation within PhylomeDB6.
*
* By default ETE4 returns duplicate faces for each of the
* "sub-trees" of the "big tree", which is incorrect.
*
* With this function we mutate the originally fetched array
* of drawing objects by removing the duplicate gene ArrowFaces starting from their second occurrences.
* @param {Array<Array<*>>} fetched_drawed_items - An array of arrays containing any type of elements.
**/
function apply_duplicate_removal_patch(fetched_drawed_items) {
    //Check if duplicated fetched_drawed_items in data fetch
    let eliminationsCount = 0;
    for (let i = 0; i < fetched_drawed_items.length; i++) {
        // Extract the Gene ID from the current item using a regex match
        let geneIdMatch = fetched_drawed_items[i][4].match(/<strong>Gene ID:<\/strong>\s*(\d+)/);
        if (!geneIdMatch) {
            continue;
        }
        const geneId = geneIdMatch[1]; // Extracted Gene ID

        // Dynamically calculate the index to check based on eliminations count
        const checkIndex = i + 7 - eliminationsCount;
        if (checkIndex > fetched_drawed_items.length - 1) {
            continue;
        }

        // Extract the Gene ID from the item at the calculated index
        let nextGeneIdMatch = fetched_drawed_items[checkIndex][4].match(/<strong>Gene ID:<\/strong>\s*(\d+)/);
        if (!nextGeneIdMatch) {
            continue;
        }

        const nextGeneId = nextGeneIdMatch[1]; // Extracted Gene ID from the next item
        if (geneId !== nextGeneId) {
            continue;
        }

        // If the Gene IDs match, delete the duplicate item
        fetched_drawed_items.splice(checkIndex, 1); // Remove the duplicate item from the array

        // Increment the deletion counter
        eliminationsCount++;

        // Reset the deletion counter if it reaches 7
        if (eliminationsCount === 7) {
            eliminationsCount = 0;
        }
    }
}
...
async function draw_aligned(params, npanels) {
    ...
    if (view.drawer.type === "rect") {
        ...
        for (let panel_n = 1; panel_n < npanels; panel_n++) {
            ...
            const items = await api(`/trees/${get_tid()}/draw?${qs}`);
            apply_duplicate_removal_patch(items)
